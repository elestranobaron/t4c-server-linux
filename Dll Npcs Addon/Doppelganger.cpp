...
void Doppelganger::OnAttacked( UNIT_FUNC_PROTOTYPE )
{
	...
	if( target != NULL )
	{
		IF(NPC_HP>2500)
			...
			ELSEIF(CheckFlag(__FLAG_NUMBER_OF_REMORTS) > 2 || CheckFlag(__FLAG_NMS_DECHU) > 0)
		...
		ELSE
			...
			CONSTANT Portals = 
				(CheckFlag(__FLAG_ADDON_PORTAL_1_CLEARED) + CheckFlag(__FLAG_ADDON_PORTAL_2_CLEARED) +
