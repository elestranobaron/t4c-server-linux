...
/******************************************************************************/
void GMAssistant::OnDeath( UNIT_FUNC_PROTOTYPE )
/******************************************************************************/
{
...
/******************************************************************************/
{
    ...
    ENDIF
        ...
        ""
        ;if ((strstr( PARAM_NPC(0), INTL( 7306, "!")) == 0) && (strstr( PARAM_NPC(0), INTL( 3771, "?")) == 0))
    {
        CHATTER_SHOUT(FORMAT(INTL( 7307, "%s."), PARAM_NPC(0)));
    }
        else
        {
            CHATTER_SHOUT( PARAM_NPC(0) )
    };
    ...
    */
    ...
    if(iMonsterNumber >= 0 && iMonsterNumber < 4)
    {
        ...
        csMonsterID[iMonsterNumber] = PARAM_NPC(2);
        Conversation
        ...
        INTL( 7328, "Accessing... ");
    {
        ...
        for( nCount = 0; nCount <= 3; nCount++)
        {
            ...
            {
                Conversation
                    FORMAT(INTL( 7329, "Monster parameter %u is set to summon %u %s(s). "), 
                    nCount + 1, iMonsterAmount[nCount], csMonsterID[nCount]);
            };
        };																					  
    }
    ...
    */
    ...
    ParamCmd(INTL( 7330, "SET PORTAL $ REWARD XP TO $"))
        "";
    CString csPortalID = PARAM_NPC(0);
    int nXP = NUM_PARAM( 1 );
    ...
    if(bPortalIndex >= 0 && bPortalIndex <= 25)
    {
        if (( nXP <= 100000 ) && (nXP >= 1))
        {			
            GiveGlobalFlag( 30320 + bPortalIndex, NUM_PARAM(1) )
                Conversation
    ...
    {
        Conversation
            FORMAT(INTL( 7333, "Error executing command. %s is not a valid dynamic portal ID. Input command:"), PARAM_NPC(0) );
    };
    ...
    if(bPortalIndex >= 0 && bPortalIndex <= 25)
    {
        if ((nGP <= 100000) && (nGP >= 1))
        {			
            GiveGlobalFlag( 30294 + bPortalIndex, nGP )
                Conversation
    ...
    if(bPortalIndex >= 0 && bPortalIndex <= 25)
    {
        ...
        if((wPositionX <= 3071 && wPositionX >= 0) && (wPositionY <= 3071 && wPositionY >= 0) && (bPositionWORLD <= 7 && bPositionWORLD >= 0))
        {
            GiveGlobalFlag( 30257 + bPortalIndex, ( ( (DWORD)( (WORD)wPositionX ) << 20 ) + ( (DWORD)( (WORD)wPositionY ) << 8 ) + (DWORD)( (BYTE)bPositionWORLD ) ) )
                Conversation
    ...
    ParamCmd(INTL( 7905, "CONVERT $ $ $ TO SANCTUARY"))
        ...
        ELSEIF ((NUM_PARAM(2) > 7) || (NUM_PARAM(2) < 0)) 
        Conversation
        INTL( 11719, "Error executing command. The world ID entered is incorrect.")
        ELSE
        DWORD nFlag;	
    IF (NUM_PARAM(0) >= 2048) 
        ;nFlag= DWORD((NUM_PARAM(0) * 1048576) + (NUM_PARAM(1) * 256) + (NUM_PARAM(2) - 4294967296)); 
    ELSE
        ;nFlag = DWORD((NUM_PARAM(0) * 1048576) + (NUM_PARAM(1) * 256) + (NUM_PARAM(2))); 
    ENDIF 
        Conversation
        ...
        Conversation
        INTL( 11723, "Processing... It is currently possible for players to summon monsters inside the colosseum.") 
        ELSE
        Conversation
        ...
        Conversation
        INTL( 20006, "Processing... the Battle Mode is now enabled. Players can fight, bringing gold and experience gains.") 
        ELSE
        Conversation
