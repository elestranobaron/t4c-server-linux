...
/******************************************************************************/
{
	InitTalk
		...
		IF(CheckGlobalFlag(__GLOBAL_FLAG_DYNAMIC_PORTAL_I) != 0)
			IF (IsInRange(4))
				UseC						  
					BYTE bPositionWorld = BYTE(CheckGlobalFlag(__GLOBAL_FLAG_DYNAMIC_PORTAL_I) & 0xFF);
					WORD wPositionY = WORD((CheckGlobalFlag(__GLOBAL_FLAG_DYNAMIC_PORTAL_I) & 0xFFF00) >> 8);
					WORD wPositionX = WORD((CheckGlobalFlag(__GLOBAL_FLAG_DYNAMIC_PORTAL_I) & 0xFFF00000) >> 20);
