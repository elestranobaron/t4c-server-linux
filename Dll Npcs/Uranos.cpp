...
IF (CheckFlag (__QUEST_URANOS) == 2 )
    ...
    "my friend!")
ELSEIF (DWORD(CheckFlag (__QUEST_URANOS)) == 1 )
	IF (DWORD(CheckNPCFlag (__URANOS_DELAY_FLAG)) > CurrentRound)
		Conversation
...
IF (CheckItem (__OBJ_DECAYING_BAT_WINGS) > 0)   
   ...
   ENDFOR
   Conversation
   FORMAT(INTL( 2083, "Thank you for the bat wings, I really appreciate your help. Here, Let me give you a small skull dagger."))

   #ifdef BUILD_NMS_CUSTOM_NPC
      ;int nExpReward = 30000;
   #else
      ;int nExpReward = 2500;
   #endif
   GiveXP(nExpReward)
