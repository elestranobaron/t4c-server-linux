...
extern NPCstructure::NPC SkraugMadlimbmanglorNPC;

void SkraugMadlimbmanglor::Create(){
   npc = SkraugMadlimbmanglorNPC;
   SET_NPC_NAME( "[10921]Skraug madlimbmanglor" );
   npc.InitialPos.X = 0; 
   npc.InitialPos.Y = 0; 
	npc.InitialPos.world = 0;
}

void SkraugMadlimbmanglor::OnAttack( UNIT_FUNC_PROTOTYPE )
{  
	switch(rnd(0, 40))	 
	{		
...
void SkraugMadlimbmanglor::OnAttacked( UNIT_FUNC_PROTOTYPE )
{ 
	switch(rnd(0, 40))	 
	{
...
void SkraugMadlimbmanglor::OnDeath( UNIT_FUNC_PROTOTYPE )
{
	switch(rnd(0, 40))	 
	{	
