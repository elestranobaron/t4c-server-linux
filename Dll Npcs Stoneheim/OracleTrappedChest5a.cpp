...
void OracleTrappedChest5a::OnAttacked( UNIT_FUNC_PROTOTYPE ){
}

void OracleTrappedChest5a::Create( void )
{
      npc = ( OracleChestNPC );
      SET_NPC_NAME( "[10840]wooden chest" );  
      npc.InitialPos.X = 2848;
      npc.InitialPos.Y = 2276;
      npc.InitialPos.world = 2;
}

void OracleTrappedChest5a::OnInitialise( UNIT_FUNC_PROTOTYPE ){
...
void OracleTrappedChest5a::OnTalk( UNIT_FUNC_PROTOTYPE )
///////////////////////////////////////////////////////////////////////////
...
IF(IsInRange(4))
	...
	ELSEIF(CheckGlobalFlag(__GLOBAL_FLAG_TRAPPED_CHEST_A) == 0)
		...
		IF(CheckGlobalFlag(__GLOBAL_FLAG_TRAPPED_CHEST_A) == 5)
		...
		ENDIF
	ELSEIF(CheckGlobalFlag(__GLOBAL_FLAG_TRAPPED_CHEST_A) == 5)
		GiveItem(__OBJ_SCORCHED_METAL_KEY)
		PRIVATE_SYSTEM_MESSAGE(INTL( 9375, "You find a scorched metal key inside the chest."))
	ELSE
		PRIVATE_SYSTEM_MESSAGE(INTL( 9353, "The chest is trapped!"))
