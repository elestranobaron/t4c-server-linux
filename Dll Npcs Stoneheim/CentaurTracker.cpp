...
extern NPCstructure::NPC CentaurTrackerNPC;

void CentaurTracker::Create(){
   npc = CentaurTrackerNPC;
   SET_NPC_NAME( "[10835]Centaur tracker" );
   npc.InitialPos.X = 0; 
   npc.InitialPos.Y = 0; 
   npc.InitialPos.world = 0;
}

void CentaurTracker::OnAttack( UNIT_FUNC_PROTOTYPE )
{  
	switch(rnd(0, 30))	 
	{		
...
void CentaurTracker::OnAttacked( UNIT_FUNC_PROTOTYPE )
{ 
		switch(rnd(0, 30))	 
	{
...
void CentaurTracker::OnDeath( UNIT_FUNC_PROTOTYPE )
{
		switch(rnd(0, 30))	 
	{	
