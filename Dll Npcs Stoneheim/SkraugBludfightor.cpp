...
extern NPCstructure::NPC SkraugBludfightorNPC;

void SkraugBludfightor::Create(){
   npc = SkraugBludfightorNPC;
   SET_NPC_NAME( "[10918]Skraug bludfightor" );
   npc.InitialPos.X = 0; 
   npc.InitialPos.Y = 0; 
	npc.InitialPos.world = 0;
}

void SkraugBludfightor::OnAttack( UNIT_FUNC_PROTOTYPE )
{  
	switch(rnd(0, 40))	 
	{		
...
void SkraugBludfightor::OnAttacked( UNIT_FUNC_PROTOTYPE )
{ 
	switch(rnd(0, 40))	 
	{
...
void SkraugBludfightor::OnDeath( UNIT_FUNC_PROTOTYPE )
{
	switch(rnd(0, 40))	 
	{	
