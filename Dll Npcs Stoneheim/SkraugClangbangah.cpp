...
extern NPCstructure::NPC SkraugClangbangahNPC;

void SkraugClangbangah::Create(){
   npc = SkraugClangbangahNPC;
   SET_NPC_NAME( "[10919]Skraug clangbangah" );
   npc.InitialPos.X = 0; 
   npc.InitialPos.Y = 0; 
	npc.InitialPos.world = 0;
}

void SkraugClangbangah::OnAttack( UNIT_FUNC_PROTOTYPE )
{  
	switch(rnd(0, 40))	 
	{		
...
void SkraugClangbangah::OnAttacked( UNIT_FUNC_PROTOTYPE )
{ 
	switch(rnd(0, 40))	 
	{
...
void SkraugClangbangah::OnDeath( UNIT_FUNC_PROTOTYPE )
{
	switch(rnd(0, 40))	 
	{	
