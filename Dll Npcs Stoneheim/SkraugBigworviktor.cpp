...
extern NPCstructure::NPC SkraugBigworviktorNPC;

void SkraugBigworviktor::Create(){
   npc = SkraugBigworviktorNPC;
   SET_NPC_NAME( "[10917]Skraug bigworviktor" );
   npc.InitialPos.X = 0; 
   npc.InitialPos.Y = 0; 
	npc.InitialPos.world = 0;
}

void SkraugBigworviktor::OnAttack( UNIT_FUNC_PROTOTYPE )
{  
	switch(rnd(0, 40))	 
	{		
...
void SkraugBigworviktor::OnAttacked( UNIT_FUNC_PROTOTYPE )
{ 
	switch(rnd(0, 40))	 
	{
...
void SkraugBigworviktor::OnDeath( UNIT_FUNC_PROTOTYPE )
{
	switch(rnd(0, 40))	 
	{	
