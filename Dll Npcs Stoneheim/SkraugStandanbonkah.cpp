...
extern NPCstructure::NPC SkraugStandanbonkahNPC;

void SkraugStandanbonkah::Create(){
   npc = SkraugStandanbonkahNPC;
   SET_NPC_NAME( "[10926]Skraug standanbonkah" );
   npc.InitialPos.X = 0; 
   npc.InitialPos.Y = 0; 
	npc.InitialPos.world = 0;
}

void SkraugStandanbonkah::OnAttack( UNIT_FUNC_PROTOTYPE )
{  
	switch(rnd(0, 40))	 
	{		
...
void SkraugStandanbonkah::OnAttacked( UNIT_FUNC_PROTOTYPE )
{ 
	switch(rnd(0, 40))	 
	{
...
void SkraugStandanbonkah::OnDeath( UNIT_FUNC_PROTOTYPE )
{
	switch(rnd(0, 40))	 
	{	
