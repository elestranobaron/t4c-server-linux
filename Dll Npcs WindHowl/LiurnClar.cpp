		...
		CmdAND(INTL( 10774, "TOME"),INTL( 10775, "VALOR"))
			...
			ELSE
				IF (CheckFlag(__QUEST_LIURN_TOME_OF_VALOR) == 0) 
					...
					ENDIF
				ELSE
					IF (CurrentRound < DWORD(CheckFlag(__QUEST_TIMER_TOME_OF_VALOR)-72000)) // There was a server crash messing up the timer:
					...
					ELSE // Server hasn't crashed, everything proceeding as normal...
						IF ((DWORD(CheckFlag(__QUEST_TIMER_TOME_OF_VALOR)) < (CurrentRound )) && (CheckFlag(__QUEST_LIURN_TOME_OF_VALOR) != 0))
							Conversation
