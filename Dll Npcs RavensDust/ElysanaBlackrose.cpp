		...
		Command(INTL( 4124, "POISON"))
			...
			SWITCH(CheckFlag(__QUEST_ELYSANA_MAKE_POISON))
				...
				CASE(1)
					...
					ENDCASE
				CASE(2)
					IF (CurrentRound < DWORD(CheckFlag(__QUEST_TIMER_MAKE_POISON)-72000)) // There was a server crash messing up the timer:
					...
					ELSE // Server hasn't crashed, everything proceeding as normal...
						IF ((DWORD(CheckFlag(__QUEST_TIMER_MAKE_POISON)) < (CurrentRound + (1 MINUTE))) && (CheckFlag(__QUEST_TIMER_MAKE_POISON) != 0))
							Conversation
