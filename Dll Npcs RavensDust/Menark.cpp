...
/******************************************************************************/
{
	...
	npc.InitialPos.X = 2862;//1342; 
	npc.InitialPos.Y = 125;//2383;
...
/******************************************************************************/
{
	...
	InitTalk
		...
		YES(THREAT)
			IF ((CheckGlobalFlag(__GLOBAL_BANK_HAS_BEEN_ROBBED_BY) == CheckFlag(__FLAG_SHADEEN_PLAYER_A)) && (DWORD(CheckGlobalFlag(__GLOBAL_BANK_HAS_BEEN_ROBBED)) > CurrentRound))
				Conversation
			...
			IF (CheckFlag(__FLAG_LIGHTBRINGER_OF_ARTHERK) == 1) 
				...
				SWITCH(CheckFlag(__MENARK_WANTS_A_POTION)) 
					...
					CASE(5)
						...
						ENDCASE
					OTHERWISE	
						PRIVATE_SYSTEM_MESSAGE(INTL( 10137, "[ERROR MENARK1.0] This text should not appear. If you see it, please report it to Dialsoft."))
						ENDCASE
				ENDSWITCH	
			ELSE
				Conversation
		...
		YES(HINT) // User wants a hint.
			int nPrice = INT(10000 * pow(double(CheckFlag(__QUEST_RIDDLES_HINTS)+1), 2.0));
			Conversation
