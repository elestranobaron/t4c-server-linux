...
/******************************************************************************/
{
	...
	npc.InitialPos.X = 1425;//1440;
	npc.InitialPos.Y = 2387;//2381;
...
/******************************************************************************/
{
	...
	InitTalk
		...
		Begin
			""
			IF ((DWORD(CheckGlobalFlag(__GLOBAL_BANK_HAS_BEEN_ROBBED)) > CurrentRound) && (CheckGlobalFlag(__GLOBAL_BANK_HAS_BEEN_ROBBED) != 0)) // Correction ici
			...
			IF (CheckFlag(__FLAG_TITLE_MORDRICK) == 1) 
				...
				ELSEIF(CheckFlag(__QUEST_FLAG_MORDRICK_STATUS) == 1)
					...
					IF(CheckFlag(__FLAG_COUNTER_SLAY_15) >= 15)
                     ...
                     ;int nExpReward = 200000+(USER_LEVEL*2500); 	
                  #else
                     ;int nExpReward = 10000+(USER_LEVEL*250); 	
                  #endif
                  GiveXP(nExpReward)
                  Conversation
				...
				ELSEIF(CheckFlag(__QUEST_FLAG_MORDRICK_STATUS) == 2)
                  ...
                  #ifdef BUILD_NMS_CUSTOM_NPC
                     ;int nExpReward = 300000+(USER_LEVEL*5000); 	
                  #else
                     ;int nExpReward = 30000+USER_LEVEL*500; 	
                  #endif
                  GiveXP(nExpReward)
                     Conversation
                     FORMAT(INTL( 4176, " {You} {have} {been} {awarded} {%u} {experience} {points.}"), nExpReward)
