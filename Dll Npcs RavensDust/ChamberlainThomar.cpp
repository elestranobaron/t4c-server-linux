...
/******************************************************************************/
{
    ...
    npc.InitialPos.X = 2833;//1417; // Castle in Silversky
    npc.InitialPos.Y = 227;//2340; // Castle in Silversky
	npc.InitialPos.world = 4;//0;
}
/******************************************************************************/
...
/******************************************************************************/
{
	...
	InitTalk
		...
		Command(INTL( 1794, "ROYAL KEY"))
			...
			ELSE // PC is a Champion of Light
				...
				ELSE // PC has received Royal Key #2 already.
					...
					IF (CurrentRound < DWORD(CheckGlobalFlag(__GLOBAL_QUEST_ROYAL_KEY2_TIMER)-72000)) // There was a server crash messing up the timer:
						...
						ENDIF
					ELSEIF ((DWORD(CheckGlobalFlag(__GLOBAL_QUEST_ROYAL_KEY2_TIMER)) <= CurrentRound) && (CheckFlag(__GLOBAL_QUEST_ROYAL_KEY2_TIMER) != 0)) // PC can receive another key again.
						Conversation
							INTL( 4571, "Very well, here is another one... Please don't lose it, this time.")
						GiveFlag(__QUEST_ROYAL_KEY2, CheckFlag(__QUEST_ROYAL_KEY2)+1)
						GiveItem(__OBJ_ROYAL_KEY_2)
		...
		Command(INTL( 896, "QUEST"))
			...
			ELSE
				IF ((DWORD(CheckNPCFlag(__NPC_THOMAR_FLAG_DELAY)) < CurrentRound) && (CheckNPCFlag(__NPC_THOMAR_FLAG_DELAY) != 0))
