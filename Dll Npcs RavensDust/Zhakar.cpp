...
{ 
	...
	ELSE
		...
		{
			case 1: 
				CastSpellSelf(__SPELL_MOB_AI_SPELL_SELF_HEALING_10_PERCENT);
	...
	InitTalk
		...
		Begin
			...
			IF(CheckFlag(__FLAG_USER_IS_WAITING_FOR_ARCANE_SPELLBOOK) != 0)
				IF(CurrentRound < DWORD(CheckFlag(__FLAG_ZHAKAR_IS_BUSY_MAKING_SPELLBOOK)))
					...
					ELSE
						...
						BREAK
					ENDIF
				ELSEIF(CurrentRound < DWORD(CheckFlag(__FLAG_ZHAKAR_IS_BUSY_MAKING_SPELLBOOK) + 18000))
					Conversation
						INTL( 7927, "It will take a few more moments, please be patient.")
					BREAK
				ELSE
		...
		Command(INTL( 7953, "RITUAL"))
			...
			ELSE
				...
				ELSEIF (CheckFlag(__QUEST_ZHAKAR_TIMER_RITUAL) != 0 )
					IF (CurrentRound < DWORD(CheckFlag(__QUEST_ZHAKAR_TIMER_RITUAL) - 72000))
						...
						ENDIF
					ELSE 
						IF ((DWORD(CheckFlag(__QUEST_ZHAKAR_TIMER_RITUAL)) < CurrentRound) && (DWORD(CheckFlag(__QUEST_ZHAKAR_TIMER_RITUAL)) != 0))
							GiveFlag(__QUEST_COMPLETED_RITUAL, 1)				
							RemFlag(__QUEST_ZHAKAR_TIMER_RITUAL)
							Conversation
