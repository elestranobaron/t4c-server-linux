...
static unsigned short Reverse16(unsigned short n)
{
...
CUDPConnection* NMPacketManager::GetConnection(sockaddr_in sockAddr,bool bCreate)
{
   for(int i=0; i<m_UDPConnectionsList.size(); i++)
   {
      ...
      if(m_UDPConnectionsList[i].sockIN.sin_port == sockAddr.sin_port)
      {
         ...
         if(NM_IPV4_ADDR(m_UDPConnectionsList[i].sockIN.sin_addr) == NM_IPV4_ADDR(sockAddr.sin_addr))
         {
...
int NMPacketManager::ReceiveUDPCallback(sockaddr_in sockAddrInIP,sockaddr_in sockAddr,unsigned char* pData, size_t DataLenght, int iQueueNbrItems, int iNbrPacketLost, void* pParam)
{
   ...
   try
   {
      if(pPacket)
      {
         ...
         {
#ifndef _WIN32
            std::fprintf(stderr, "[UDP] checksum FAIL from %s:%u\n",
...
#ifndef _WIN32
     {
        extern void DbgRegisterReceiveThreadForStallDump();
        DbgRegisterReceiveThreadForStallDump();
     }
#endif
     ENTER_TIMEOUT
...
inline void NMPacketManager::PostReceivePacket (UDPPacket* pPacket)
{
   ...
   PostQueuedCompletionStatus( m_hUDPReceivePacketIO, 0, reinterpret_cast< std::uintptr_t >( pPacket ), NULL );
}

inline void NMPacketManager::PostSendPacket (UDPPacket* pPacket) 
{
   PostQueuedCompletionStatus( m_hUDPSendPacketIO, 0, reinterpret_cast< std::uintptr_t >( pPacket ), NULL );
}
...
inline void NMPacketManager::PostSendPacketMega (UDPPacket* pPacket) 
{
   ...
   PostQueuedCompletionStatus( m_hUDPSendPacketIO, 0, reinterpret_cast< std::uintptr_t >( pMegaPack ), NULL );
}

inline void NMPacketManager::PostLostConnection  (CUDPConnection* pConnection)
{
   PostQueuedCompletionStatus( m_hLostConnIO, 0, reinterpret_cast< std::uintptr_t >( pConnection ), NULL );
}
