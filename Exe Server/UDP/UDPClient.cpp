...
int UDPClient::StartServer(LPCTSTR Address, int Port, bool bBuffered)
{
...

    {
       ...
       setsockopt( hSocket,SOL_SOCKET, SO_REUSEADDR, (char *) &optval , sizeof( BOOL ));
       setsockopt( hSocket,SOL_SOCKET, SO_BROADCAST, (char *) &optval , sizeof( BOOL ));
       setsockopt( hSocket,SOL_SOCKET, SO_RCVBUF   , (char *) &bufsize, sizeof bufsize);
       setsockopt( hSocket,SOL_SOCKET, SO_SNDBUF   , (char *) &bufsize, sizeof bufsize);
    }
    ...
    if (bind(hSocket, (SOCKADDR *) & m_sockAddrLocal1, sizeof (m_sockAddrLocal1)) == 0)
    {
        ...
        UINT threadId;
        m_hReceiveThread1 = (HANDLE)_beginthreadex( NULL, 0, fReceiveData1 , this, 0, &threadId );
...
#else
    (void)bBuffered;
    std::memset(&m_sockAddrSender1, 0, sizeof(m_sockAddrSender1));
    if (NM_BindUdpSocket(Address, Port, m_pSocket1, m_sockAddrLocal1) != 0) {
        return eERROR_INITIALIZATION;
    }
    unsigned threadId = 0;
    m_hReceiveThread1 = (HANDLE)_beginthreadex(NULL, 0, fReceiveData1, this, 0, &threadId);
...
int UDPClient::StartServer2(LPCTSTR Address, int Port, bool bBuffered)
{
   ...
   if (bind(hSocket, (SOCKADDR *) & m_sockAddrLocal2, sizeof (m_sockAddrLocal2)) == 0)
   {
      ...
      UINT threadId;
      m_hReceiveThread2 = (HANDLE)_beginthreadex( NULL, 0, fReceiveData2 , this, 0, &threadId );
...
#else
   (void)bBuffered;
   std::memset(&m_sockAddrSender2, 0, sizeof(m_sockAddrSender2));
   if (NM_BindUdpSocket(Address, Port, m_pSocket2, m_sockAddrLocal2) != 0) {
      return eERROR_INITIALIZATION;
   }
   unsigned threadId = 0;
   m_hReceiveThread2 = (HANDLE)_beginthreadex(NULL, 0, fReceiveData2, this, 0, &threadId);
...
int UDPClient::SendTo(sockaddr_in sockToO,sockaddr_in sockToI,char* pBuff, size_t BuffLenght)
{
   ...
   return eNO_ERROR;
#else
   NMSocket *pSock = NM_SameIPv4(sockToI, m_sockAddrLocal1) ? m_pSocket1 : m_pSocket2;
   if (!pSock) {
      return eERROR_COULD_NOT_SEND;
   }
   const ssize_t sent = ::sendto(pSock->m_hSocket, pBuff, BuffLenght, 0,
      reinterpret_cast<sockaddr *>(&sockToO), sizeof(sockToO));
   if (sent < 0) {
      EnterCriticalSection(&crLost);
      vConnectionLost.push_back(sockToO);
      LeaveCriticalSection(&crLost);
...
ULONG UDPClient::GetIPAddress( LPCTSTR strHostName )
{
   ...
   ULONG uAddr = ::inet_addr(strHostName);
   ...
   if (getaddrinfo(strHostName, nullptr, &hints, &res) != 0 || res == nullptr) {
      return INADDR_NONE;
   }
   const auto *sa = reinterpret_cast<sockaddr_in *>(res->ai_addr);
   uAddr = ntohl(sa->sin_addr.s_addr);
   freeaddrinfo(res);
   return uAddr;
#else
   LPHOSTENT   lphostent;
...
bool UDPClient::GetLocalName(LPTSTR strName, UINT nSize)
{
   ...
   addrinfo *res = nullptr;
   if (getaddrinfo(host, nullptr, &hints, &res) == 0 && res != nullptr && res->ai_canonname != nullptr) {
      std::strncpy(strName, res->ai_canonname, nSize - 1);
      strName[nSize - 1] = '\0';
      freeaddrinfo(res);
      return true;
   }
   if (std::strlen(host) >= nSize) {
      errno = ERANGE;
      return false;
   }
   std::strncpy(strName, host, nSize - 1);
   strName[nSize - 1] = '\0';
   return true;
#else
   if (strName != NULL && nSize > 0)
...
UINT UDPClient::fReceiveData1(LPVOID pParameter)
{
    ...
    while (me->m_bReceiveThread1) {
        ...
        const ssize_t n = ::recvfrom(me->m_pSocket1->m_hSocket, pData, PACKET_MAX_SIZE, 0,
        ...
        BOOL bSkip = FALSE;
        if (n >= 4) {
            PacketHeaderDebug *pHeader = reinterpret_cast<PacketHeaderDebug *>(pData);
            if (pHeader->ushCompress || pHeader->uchPartNumber > 32) {
                bSkip = TRUE;
            }
        }
        if (!bSkip && me->m_pFcnRecvData) {
            EnterCriticalSection(&me->m_crLockCallbackMultipleIP);
            try {
                (*me->m_pFcnRecvData)(me->m_sockAddrLocal1, sender, pData, static_cast<size_t>(n),
                    0, 0, me->m_pFcnRecvParam);
            } catch (...) {
            }
...
UINT UDPClient::fReceiveData2(LPVOID pParameter)
{
   ...
   while (me->m_bReceiveThread2) {
      sockaddr_in sender {};
      socklen_t addrLen = sizeof(sender);
      const ssize_t n = ::recvfrom(me->m_pSocket2->m_hSocket, pData, PACKET_MAX_SIZE, 0,
         reinterpret_cast<sockaddr *>(&sender), &addrLen);
      if (n <= 0) {
         Sleep(1);
         continue;
      ...
      if (me->m_pFcnRecvData) {
         EnterCriticalSection(&me->m_crLockCallbackMultipleIP);
         try {
            (*me->m_pFcnRecvData)(me->m_sockAddrLocal2, sender, pData, static_cast<size_t>(n),
               0, 0, me->m_pFcnRecvParam);
         } catch (...) {
         }
         LeaveCriticalSection(&me->m_crLockCallbackMultipleIP);
