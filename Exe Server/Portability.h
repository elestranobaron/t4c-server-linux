...
public:
    ...
    /* MFC CString(LPCTSTR) accepte NULL (= chaine vide) ; std::string(NULL) plante. */
    String(const char *s) : std::string(s ? s : "") {}
    String(const unsigned char *s) : std::string(reinterpret_cast<const char *>(s ? s : "")) {}
    ...
    char *GetBuffer(int minLen) {
        ...
        /* MFC ne retourne JAMAIS NULL (meme chaine vide) : retourner nullptr faisait
         * planter le talk PNJ (strMessage vide -> strlen(NULL) / CString(NULL)). */
        return &(*this)[0];
    }
    char *GetBuffer() {
        return GetBuffer(0);
    ...
    void GetBufferSetLength(int nNewLength) {
        if (nNewLength < 0) {
            nNewLength = 0;
        }
