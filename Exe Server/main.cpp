...
#else
	const char last = csText.GetAt(csText.GetLength() - 1);
	if (last != '/' && last != '\\') {
		csText += '/';
	}
#endif
...
#ifndef _WIN32
	csT4CKEY = "Software\\Vircom\\The 4th Coming Server\\";
#endif
	...
	{        
	  ...
	  else
	  {
        ...
        theApp.csDBPwd  = "";
		  ...
		  theApp.sAuth.csODBC_Table = "T4CUsers";
	  ...
	  // Ini [Characters] DB_USER/DB_PWD when AUTH_DB_* / ODBC_DB_* are unset (Linux port).
	  if( regKey.Open( HKEY_LOCAL_MACHINE, theApp.csT4CKEY+CHARACTER_KEY ) )
	  {
		  if( theApp.csDBUser.IsEmpty() )
			  KEY2TXT( theApp.csDBUser, "DB_USER", "" );
		  if( theApp.csDBPwd.IsEmpty() )
			  KEY2TXT( theApp.csDBPwd, "DB_PWD", "" );
		  if( theApp.sAuth.csODBC_DBUser.IsEmpty() )
			  theApp.sAuth.csODBC_DBUser = theApp.csDBUser;
		  if( theApp.sAuth.csODBC_DBPwd.IsEmpty() )
			  theApp.sAuth.csODBC_DBPwd = theApp.csDBPwd;
		  regKey.Close();
	  }
     ...
     _LOG_EVENTS  LOG_ALWAYS, "Starting T4C server." LOG_
		...
		const bool serverPathExists = std::filesystem::exists(
		    std::filesystem::path(static_cast<const char *>(static_cast<LPCTSTR>(ServerPath))), pathEc);
#endif
		if (!serverPathExists) {
...
{ 
	...
	try
	{
...
#ifndef _WIN32
        fprintf(stderr, "[MAIN] post-WDA init…\n");
        fflush(stderr);
#endif
     ...
     _LOG_DEBUG
        ...
        LOG_   
#ifndef _WIN32
        fprintf(stderr, "[MAIN] CPacketManager::Create…\n");
        fflush(stderr);
#endif
...
#ifndef _WIN32
      Character::LogStartupItems();
      fprintf(stderr, "[BOOT] UDP comm enabled (port ready for auth)\n");
      fflush(stderr);
#endif
...
#ifndef _WIN32
      fprintf(stderr, "[BOOT] NPC world spawn done (MSG_OnServerInitialisation)\n");
      fprintf(stderr, "[BOOT] SERVER_READY\n");
      fflush(stderr);
#endif
