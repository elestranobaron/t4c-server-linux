...
/******************************************************************************/
{
	if ( boIgnoreMaxWeight == false && GetFreeWeight() < oObjectToAdd->GetWeight() * oObjectToAdd->GetQty() )
	{
		return FALSE;
	}
...
/******************************************************************************/
// void GetPacket (TFCPacket) : add container info to the packet
void ItemContainer::GetPacketNew(TFCPacket &packet,WORD wlang)
/******************************************************************************/
{
   if (container->NbObjects() > 0) 
   {
      ...
      while (container->QueryNext()) 
      {
         ...
         obj->SendUnitMessage( MSG_OnGetUnitStructure, NULL, NULL, NULL, NULL, &item );
         if( item != NULL )
            lEquipedPos = item->equip_position;
         packet << (long)lEquipedPos;
