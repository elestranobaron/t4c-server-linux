...
public:
   ...
   /** PickLock court pour paquets move (1–8) — evite stall UDP 5000 tours. */
   static Players *GetPlayerResourceFctForMove( const sockaddr_in &sockAddr );
   /** PickLock court preInGame (46, 60…) — sans exiger in_game. */
   static Players *GetPlayerResourceFctForSession( const sockaddr_in &sockAddr );
   ...
   /** Marque pour suppression les sessions fantome (meme compte, autre socket). */
   static void FlagDeleteByAccount( const CString &csAccountName );

   /** True si le compte a encore une session UDP enregistree cote serveur. */
   static BOOL IsAccountRegistered( const CString &csAccountName );

   /** Purge synchrone d'une session fantome (reconnexion meme IP/port). */
   static void ForceReconnectPurge( const sockaddr_in &sockAddr );

   /** Purge synchrone par nom de compte (client relance, nouveau port UDP). */
   static void ForceReconnectPurgeByAccount( const CString &csAccountName );

   /** Retire toutes les sessions d'un compte sauf le socket keepAddr (reconnect apres monde). */
   static void PurgeAccountSessionsExcept( const CString &csAccountName, const sockaddr_in &keepAddr );

   /** Retire immediatement une session deja flag delete sur ce socket (Register reconnect). */
   static void DropFlaggedSessionAt( const sockaddr_in &sockAddr );

   /** Reutilise le slot UDP (ExitGame + Register) sans delete async / ODBC. */
   static Players *ResetFlaggedSessionForReauth( const sockaddr_in &sockAddr );

   /** Lookup IP/port sans PickLock (thread UDP Register — evite blocage in_game). */
   static Players *PeekPlayerAtEndpoint( const sockaddr_in &sockAddr );
