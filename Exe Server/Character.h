...
//	void save_character();
   ...
   BOOL CastSpellPosSystem( WORD wSpellID, WorldPos wlPos );
    ...
    /* DOIT matcher Unit::SendPrivateMessage (const CString &) sinon ce n'est pas un
     * override : l'appel virtuel via Unit* tombait sur le corps vide de Unit et les
     * reponses privees des PNJ (dialogues) n'etaient jamais envoyees au client. */
    virtual void SendPrivateMessage( const CString &csMessage, Unit *lpuUnit, DWORD dwColor = CL_CYAN ) override;
   ...
   static void LogStartupItems( void );
...
//    WORD wLang;
    ...
    DWORD MaxHP;
    DWORD HP;
    /* PV juste avant le coup fatal : a la resurrection on rend ces PV (T4C 1.25)
     * au lieu du plein de vie. */
    DWORD dwHPBeforeLastBlow;
    DWORD dwLastHealing;
