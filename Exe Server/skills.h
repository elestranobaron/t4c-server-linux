...
#define NB_SKILL_HOOKS		9		// Quantity of possible hooks defined below
...
#define Hook_OnSpellAttack	7		// When player attack with a spell.
#define	Hook_OnSpellAttacked 8		// When player gets attacked by a spell.
...
#define HOOK_SPELL_ATTACK		0x00000400	// Auto-skill use on attacks with a spell
#define HOOK_SPELL_ATTACKED		0x00000800	// Auto-skill use when attacked with a spell
...
typedef struct _SKILL_ATTRIBUTES
{
 //8/Powerful Blow:   Warriors.  10  5-1000 AU    09  STR: 35.           25W.
	int skLevel;
	int skAGI, skSTR, skEND, skINT, skWIS, skWIL, skLCK;
	TemplateList <USER_SKILL> tlskSkillRequired;
...
#define ADD_REQUIRED_SKILL( _Skill, _Strength )	{LPUSER_SKILL lpUserSkill = new USER_SKILL;\
	...
	s_saAttrib.tlskSkillRequired.AddToTail( lpUserSkill );\
}
...
typedef struct _SKILL
{
    ...
    LPCTSTR __declspec(dllexport) GetName( WORD wLanguage );
    void SetName( const char *lpszName )
    {
        strcpy_s( lpszSkillName, 60, lpszName );        
    }
...
/******************************************************************************/
// The class
class __declspec(dllexport) Skills
/******************************************************************************/
{
public:
