...
{    
   ...
   target = self;
    ...
    if( self->ViewFlag( __FLAG_PRIMALSCREAM_EXHAUST ) == 0 ) 
    {
		...
		// La competence ne peut pas ùchouer
		dFinalSuccess = 100;
		...
		if(lpCharS->GetMana() < iManaCost) 
      {
		...
		// Test de la competence (Passe toujours pour l'instant, voir si je met un test aprùs les tests)
		int iSuccess = rnd( 0, 100 );

		if(  iSuccess <= (int)dFinalSuccess && dFinalSuccess >0) 
      {
         // AGRO DES MOBS DANS UN RAYON DE skill/20 bornù entre 2 (0 pts) et 10 (200 pts)
