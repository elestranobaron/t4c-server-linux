...
Add Profession,Karma Crime et macro NPC Typhia reroll et remort... Formule List by NightMare (26/06/2009)
/******************************************************************************/
...
void EXPORT RemortDechu1To( Unit *self, DWORD x, DWORD y, DWORD world );
void EXPORT RemortDechu2To( Unit *self, DWORD x, DWORD y, DWORD world );
...
#define REMORTDECHU1_TO( ixe, why, world ) RemortDechu1To( target, ixe, why, world );
#define REMORTDECHU2_TO( ixe, why, world ) RemortDechu2To( target, ixe, why, world );
#define REMORT_DIRECT_TO( ixe, why, world, iE, iX ) RemortDirectTo( target, ixe, why, world, iE, iX );
#define REROLL_DIRECT( ixe, why, world) RerollDirect( target, ixe, why, world);
...
/*
: Ajout du syst�me multilingue.
					   C'est une modification qui prend en compte la langue de l'utilisateur et non celle de l'unit� elle m�me.
#define INTL(ID, X)		   IntlText::GetString( ID, ( target == NULL ? _DEFAULT_LNG : target->GetLang() ), X ) */
#define INTL(ID, X)        IntlText::GetString( ID, ( static_cast< Character * >( target ) == NULL ? _DEFAULT_LNG : static_cast< Character * >( target )->GetLang() ), X )
...
#define PARAM_NPC( __num )		lpcsParams[ __num ]
...
//#define C( __num )	;output += _itoa( __num, lpszDummy, 10 );\
//					output += 
#define C( __num )	; _itoa_s( __num, lpszDummy,16, 10 );\
					output +=lpszDummy;\
					output += 
...
#define BroadcastFriendlyUpdate(wl , mID) TFCMAIN::BroadcastFriendlyChange(wl, mID)

#define PLFactionMask( val )            TFCMAIN::GetPLFactionMask(val)
#define CheckGlobalFlag( flag )         TFCMAIN::GlobalFlagView(flag)
#define GiveGlobalFlag( flag, value )   ;TFCMAIN::GlobalFlagSet( flag, value ,TRUE)
#define RemGlobalFlag( flag )           ;TFCMAIN::GlobalFlagDel( flag)
#define AddCastSpellPosition(self,wl,SpellID,Freq,repeat); TFCMAIN::AddCastSpellPos( self,wl,SpellID,Freq,repeat)
...
#define ACK_COLOSSEUM2		TFCMAIN::GetColosseum2()
#define ACK_COLOSSEUM3		TFCMAIN::GetColosseum3()
#define ACK_COLOSSEUM4		TFCMAIN::GetColosseum4()
...
BOOL EXPORT __GiveItem( Unit *self, Unit *target, WORD wItemID, BOOL boEcho = TRUE, bool boGiveAbsolute = true, bool boBackpackUpdate = true , bool bLog = true);
#define GiveItem(item)		 ;__GiveItem( self, target, item, false, true, false ,true);xSendBackpackUpdate = true;

/******************************************************************************/

/******************************************************************************/
// Give item with no update, not absolute
#define GiveItemNoUpdateNotAbsolute( item ) (xSendBackpackUpdate=true,__GiveItem( self, target, item, false, false, false ,false))
...
#define GiveCrime( amount ) ;target->SetCrime( target->GetCrime() + amount );
#define GiveHonor( amount ) ;target->SetHonor( target->GetHonor() + amount );
...
/******************************************************************************/
#define CreateFormuleList		;{  BOOL boCantLearn = TRUE;\
								      TemplateList <_FORMULEITEM> tlFormuleList;
...
#define USER_CRIME  target->GetCrime()
#define NPC_CRIME   self->GetCrime()
#define SET_CRIME(__cnt)   self->->SetCrime( __cnt );

#define USER_HONOR  target->GetHonor()
#define NPC_HONOR   self->GetHonor()
#define SET_HONOR(__cnt)   ;self->->SetHonor( __cnt );

#define USER_LUCK   target->GetLCK()
#define USER_TRUE_LUCK   target->GetTrueLCK()
#define NPC_LUCK   self->GetLCK()
#define NPC_TRUE_LUCK   self->GetTrueLCK()
...
/******************************************************************************/
void EXPORT AddTeachFormuleFunc( int formuleID, DWORD formuleCost, BOOL &boCantLearn, TemplateList< _FORMULEITEM > &tlFormuleList, Unit *target );
#define AddTeachFormule( THE_FORMULE, __Cost ) ;AddTeachFormuleFunc( THE_FORMULE, __Cost, boCantLearn, tlFormuleList, target );
/******************************************************************************/
...
#define SendTeachFormuleList  ;SendTeachFormuleListFunc( boCantLearn, tlFormuleList, target ); };

void EXPORT SendAHListFunc(Unit *target );
#define SendAHList  ;SendAHListFunc( target );
/******************************************************************************/
...
// Sends a trade points list to the client
void EXPORT SendPointsItemListFunc( TemplateList< _OBJECTITEM > &tlItemList, Unit *target );
#define SendPointsItemList	;SendPointsItemListFunc( tlItemList, target ); };
...
/******************************************************************************/
// Handle a teaching block
#define NPC_DATAF_TEACH		case __TEACH_DATAF:{ BOOL FormuleTaught = FALSE;\
								LPTEACH_DATAF TeachDataF = (LPTEACH_DATAF)dataIN->Data;
								

// Init a list of skills to be taught
#define NPC_TEACH_FORMULE_LIST		switch( TeachDataF->wFormuleID ){

#define CLOSE_NPC_TEACH_FORMULE_LIST	};

BOOL EXPORT __TeachProfession(Unit *lpuWho, DWORD dwID, DWORD dwGoldCost );
#define TEACH_PROFESSION_FORMULE( THE_FORMULE, __Cost )  case THE_FORMULE :\
      if( Professions::IsFormuleExist(THE_FORMULE) ){\
			 if( Professions::IsFormuleLearnable(THE_FORMULE,target) ==1){\
				FormuleTaught = __TeachProfession( target, THE_FORMULE, __Cost ); \
			}\
		}\
      break;
...
#define NPC_SELL(price, item)		;}else if(shop->Action == __BUY && shop->Item == item){\
                              ...
                              if(iNbrSold > 0){\
                                 char lpszID[ 256 ];\
                                 Unit::GetNameFromID(item, lpszID, U_OBJECT );\
                                 _LOG_NPCS\
                                    LOG_MISC_1,\
