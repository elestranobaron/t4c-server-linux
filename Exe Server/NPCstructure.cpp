...
/******************************************************************************/
{
	...
	if(npc.appearance != -1)
	{
		...
		if(world)
		{
			Unit *newNPC = world->create_world_unit(U_NPC, wBaseReferenceID, npc.InitialPos, NULL, FALSE);
			/* PNJ a position fixe (banquier/marchand derriere un comptoir = tuile souvent
			 * marquee bloquante dans map_buffy cote Linux) : on force le spawn pour ne pas
			 * perdre Mithrand & co. comme sur le client Windows d'origine. */
			if(!newNPC && npc.InitialPos.X != 0 && npc.InitialPos.Y != 0)
			{
				newNPC = world->create_world_unit(U_NPC, wBaseReferenceID, npc.InitialPos, NULL, TRUE);
#ifndef _WIN32
				if(newNPC)
				{
					char szDbgID[ 256 ];
					if(!Unit::GetNameFromID( wBaseReferenceID, szDbgID ))
						sprintf_s(szDbgID,256,"?");
					std::fprintf( stderr,
					              "[BOOT] NPC force-spawn \"%s\" ID=%s @ %u,%u w=%u (tuile bloquante)\n",
					              npc.name != NULL ? npc.name : "?", szDbgID,
					              static_cast<unsigned>( npc.InitialPos.X ),
					              static_cast<unsigned>( npc.InitialPos.Y ),
					              static_cast<unsigned>( npc.InitialPos.world ) );
					std::fflush( stderr );
				}
#endif
			}
...
#ifndef _WIN32
				{
					std::fprintf( stderr, "[BOOT] NPC spawn \"%s\" unitID=%lu app=%d @ %u,%u w=%u\n",
					              npc.name != NULL ? npc.name : "?",
					              static_cast<unsigned long>( newNPC->GetID() ),
					              static_cast<int>( npc.appearance ),
					              static_cast<unsigned>( npc.InitialPos.X ),
					              static_cast<unsigned>( npc.InitialPos.Y ),
					              static_cast<unsigned>( npc.InitialPos.world ) );
					std::fflush( stderr );
...
#ifndef _WIN32
               std::fprintf( stderr,
                             "[BOOT] NPC spawn FAILED \"%s\" ID=%s @ %u,%u w=%u\n",
                             npc.name != NULL ? npc.name : "?",
                             szID,
                             static_cast<unsigned>( npc.InitialPos.X ),
                             static_cast<unsigned>( npc.InitialPos.Y ),
