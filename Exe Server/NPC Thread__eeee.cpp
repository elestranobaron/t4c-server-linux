...
/******************************************************************************/
{
   ...
   while( !boNPCThreadDone ) // Main NPC Loop
   {
      ...
      while(IsNearSubmission.QueryNext())
      {
         ...
         //CV:Crash:NPCThread: Ajout d'un null et validation etre sure la targer ets valides...
         ThisCreature = NULL;
      ...
      while( AddCreatureList.QueryNext() )
      {
         ...
         if(AddCreatureList.Object())//CV:Crash:NPCThread:on ajoute pas une creature NULL au cas ou....
      ...
      while(CreatureList.QueryNext())
      {
         ...
         if(ThisCreature)
         {
            ...
            while(tluElectricChair.QueryNext())
            {
               ...
               if(cuCondemned)
                  ThisCreature->RemoveReferenceTo( cuCondemned ); //CV:Crash:NPCThread: Avant commenter... remis et add validation pointeur OK
            }
            ...
            else
            {
               ...
               if( ThisCreature->IsDoing() == fighting && ThisCreature->GetTarget() != NULL )
               {
                  ...
                  if( ThisCreature->GetTarget()->GetType() == U_PC )
                  {
                     ...
                     if( lpPlayer != NULL && ((lpPlayer->GetGodFlags() & GOD_NO_MONSTERS) || (lpPlayer->self->ViewFlag( __FLAG_NMS_PLAYER_DEATH ) != 0)))
                     {
