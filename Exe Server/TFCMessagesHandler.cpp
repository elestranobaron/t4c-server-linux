...
/******************************************************************************/
/* LP64 : TFCPacket::Get(long*) ecrit sizeof(long)=8 octets sur Linux 64 bits.
 * L'appeler via un cast (long*)&dword32 ecrase la variable voisine sur la pile
 * (ex. RQ_DirectedTalk : 'where' lue puis ecrasee par Get de theID/color →
 * FindNearUnit recevait une position corrompue → RQ_BreakConversation).
 * Toujours lire un champ 32 bits via un int32 temporaire. */
#define GET_PACKET_U32(msgp, var)            \
   do {                                      \
      std::uint32_t _v32 = 0;                \
      (msgp)->Get(&_v32);                    \
      (var) = static_cast<DWORD>(_v32);      \
   } while(0)
...
#define RQ_FOOTER( rq_name )   }catch( ... ){\
                                _LOG_DEBUG\
                                    LOG_ALWAYS,\
                                    "Detected crash in %s", rq_name\
                                LOG_\
                                _LOG_DEBUG\
                                    LOG_ALWAYS,\
                                    "Packet: [ %s ]",\
...
/******************************************************************************/
{
   ...
   RegisterFunction( RQFUNC_PlayerMove,					   RQ_MoveNorthWestKB );
	...
	RegisterFunction( RQFUNC_SafePlug,					   	(RQ_SIZE)123 );
...
#ifndef _WIN32
	// Linux: même DSN/credentials que ODBCUsers (connexion auth dédiée souvent sans lignes).
	ODBCAuth.Connect( USERS_DSN,
	                  theApp.csDBUser.IsEmpty() ? (LPCTSTR)NULL : (LPCTSTR)theApp.csDBUser,
	                  theApp.csDBPwd.IsEmpty() ? (LPCTSTR)NULL : (LPCTSTR)theApp.csDBPwd );
#else
...
bool LoadPlayer(LPRQSTRUCT_REGISTER_ACCOUNT lpStruct,CString &csErrorMsg,TFCPacket &sending, long &LongKeyCode)
{
   ...
   Players *lpPlayer = lpStruct->lpReusePlayer;
   if( lpPlayer == NULL )
      lpPlayer = CPlayerManager::CreatePlayer( lpStruct->sockAddrO,lpStruct->sockAddrI, lpStruct->csAccount );
   else
      lpPlayer->Lock();
   if( lpPlayer != NULL )
   {
      ...
      lpPlayer->SetLang( lpStruct->usLangUsed ); // Ajout du syst�me multilingue.
...
#if !defined( _WIN32 )
static void AsyncRQFUNC_FromPreInGameToInGame( LPVOID lpData )
{
   ...
   if( user == NULL || user->self == NULL || user->IsDeleteFlags() )
   {
      std::fprintf( stderr,
                    "[FromPreInGameToInGame] async abandon (user invalid) params=%p\n",
                    static_cast<void *>( lpParams ) );
      delete lpParams;
      return;
   }
   ...
   if( !user->boPreInGame || user->in_game )
   {
      ...
      std::fprintf( stderr,
                    "[FromPreInGameToInGame] deja in_game pour %s (preInGame=%d in_game=%d)\n",
                    (LPCTSTR)user->GetAccount(),
                    user->boPreInGame ? 1 : 0,
                    user->in_game ? 1 : 0 );
      std::fflush( stderr );
      return;
   }
   ...
   std::fprintf( stderr,
                 "[FromPreInGameToInGame] async PutPlayerInGame pour %s\n",
                 (LPCTSTR)user->GetAccount() );
   std::fflush( stderr );
...
#if !defined( _WIN32 )
static void AsyncRQFUNC_GetNearItems( LPVOID lpData )
{
   LPASYNC_PACKET_FUNC_PARAMS lpParams = static_cast<LPASYNC_PACKET_FUNC_PARAMS>( lpData );
   if( lpParams == NULL || lpParams->user == NULL || lpParams->user->self == NULL
       || lpParams->user->IsDeleteFlags() )
   {
      delete lpParams;
      return;
   }
   ...
   if( !user->UsePicklock( __FILE__, __LINE__ ) )
   {
      TFCPacket emptyReply;
      emptyReply << (RQ_SIZE)RQ_GetNearItems;
      if( user->self != NULL )
         user->self->SendPlayerMessage( emptyReply );
      delete lpParams;
      return;
   }

   struct AutoUnlockNear
   {
      Players *u;
      explicit AutoUnlockNear( Players *p ) : u( p ) {}
      ~AutoUnlockNear()
      {
         if( u != NULL )
            u->UseUnlock( __FILE__, __LINE__ );
      }
   } autoUnlock( user );
   ...
   if( user->boPreInGame || user->in_game )
   {
      ...
      {
         TFCPacket sending;
         const int read = lpWorld->packet_inview_units( wlPos, sending, _DEFAULT_RANGE, user->self );
         if( read > 0 )
...
void AsyncRQFUNC_PutPlayerInGame( LPVOID lpData ) 
{
   ...
   AutoExit cAutoExit( user, lpParams );

   if( user->in_game )
      return;

#ifndef _WIN32
   std::fprintf( stderr, "[PutPlayerInGame] async load « %s » compte %s\n",
                 (LPCTSTR)lpParams->strParam, (LPCTSTR)user->GetAccount() );
#endif
   _LOG_DEBUG
      LOG_DEBUG_LVL1,
...
#ifndef _WIN32
         std::fprintf( stderr, "[PutPlayerInGame] load_character(%s) -> code %d\n",
                       (LPCTSTR)lpParams->strParam, static_cast<int>( receive ) );
#endif
         _LOG_DEBUG
            LOG_DEBUG_LVL1,
   ...
   if(player_pos.world < TFCMAIN::GetMaxWorlds())
   {
      ...
      if( world != NULL && world->IsValidPosition(player_pos) )
      {
         ...
         {	
            ...
            user->ResetIdle();
            std::fprintf( stderr,
                          "[PutPlayerInGame] picklock relache apres 13 (46/60) pour « %s » preInGame=%d in_game=%d\n",
                          (LPCTSTR)lpParams->strParam,
                          user->boPreInGame ? 1 : 0,
                          user->in_game ? 1 : 0 );
            std::fflush( stderr );
         ...
         {
            TFCPacket errReply;
            errReply << (RQ_SIZE)RQ_PutPlayerInGame;
            errReply << receive;
            user->self->SendPlayerMessage( errReply );
         }
      }
      else
      {
#ifndef _WIN32
         std::fprintf( stderr,
                       "[PutPlayerInGame] position monde invalide « %s » wl=%u,%u,%u\n",
                       (LPCTSTR)lpParams->strParam,
                       static_cast<unsigned>( player_pos.X ),
                       static_cast<unsigned>( player_pos.Y ),
                       static_cast<unsigned>( player_pos.world ) );
#endif
         TFCPacket errReply;
         errReply << (RQ_SIZE)RQ_PutPlayerInGame;
         errReply << (char)1;
...
void TFCMessagesHandler::AsyncRQFUNC_RegisterAccountODBC( LPVOID lpData)
{
...
#ifndef _WIN32
   std::fprintf( stderr, "[AUTH] Async RegisterAccount demarre compte='%s' IP %s:%u\n",
                 (LPCTSTR)lpStruct->csAccount,
                 inet_ntoa( lpStruct->sockAddrO.sin_addr ),
                 static_cast<unsigned>( ntohs( lpStruct->sockAddrO.sin_port ) ) );
#endif

   Players::DeleteOnlineUserSync( lpStruct->csAccount );
   if( lpStruct->lpReusePlayer == NULL )
      CPlayerManager::PurgeAccountSessionsExcept( lpStruct->csAccount, lpStruct->sockAddrO );
   ...
   accountName = lpStruct->csAccount;
   ...
   {
      lpStruct->csAccount = accountName.Left(20);
      blogCheat = true;
   }
   else
   {
      lpStruct->csAccount = accountName;
   }
   ...
   {
#ifndef _WIN32
      std::fprintf( stderr, "[AUTH] reject: account len=%d\n", lpStruct->csAccount.GetLength() );
#endif
      bOK = false;
   ...
   {
      // Do authentification (AccountLogged stale : deja purge plus haut)
      CString csQuery;
      BOOL boAuth = TRUE;
...
#ifndef _WIN32
      if( !Players::FetchAuthPassword( lpStruct->csAccount, szPassword, (int)sizeof( szPassword ) ) )
      {
...
#else
      ODBCAuth.Lock();
      csQuery.Format("SELECT %s FROM %s WHERE %s='%s'",
      ...
      if( boAuth )
      {
         if( !Players::ClaimOnlineSlot( lpStruct->csAccount, inet_ntoa( lpStruct->sockAddrO.sin_addr ) ) )
         {
#ifndef _WIN32
            std::fprintf( stderr, "[AUTH] reject: ClaimOnlineSlot('%s')\n", (LPCTSTR)lpStruct->csAccount );
#endif
            bOK = false;
...
#ifndef _WIN32
   std::fprintf( stderr, "[AUTH] RegisterAccount reponse compte='%s' ok=%d\n",
                 (LPCTSTR)lpStruct->csAccount, bOK ? 1 : 0 );
#endif
...
void TFCMessagesHandler::RQFUNC_AuthenticateServerVersion( PACKET_FUNC_PROTOTYPE )
{
   RQ_HEADER;
#ifndef _WIN32
   std::fprintf( stderr, "[AUTH] AuthenticateServerVersion (99) compte='%s' IP %s:%u registred=%d delete=%d\n",
                 user ? (LPCTSTR)user->GetAccount() : "?",
                 inet_ntoa( sockAddrO.sin_addr ),
                 static_cast<unsigned>( ntohs( sockAddrO.sin_port ) ),
                 user && user->registred ? 1 : 0,
                 user && user->IsDeleteFlags() ? 1 : 0 );
#endif
...
void TFCMessagesHandler::RQFUNC_Shout(PACKET_FUNC_PROTOTYPE)
{
   ...
   //Correction contre la faille du log cc sans �tre en jeu...	
...
void TFCMessagesHandler::RQFUNC_DirectedTalk(PACKET_FUNC_PROTOTYPE)
{
   ...
   if(user->in_game)
   {
      ...
      msg->Get((short *)&where.X);
      msg->Get((short *)&where.Y);			
      GET_PACKET_U32(msg, theID);
      msg->Get((char  *)&bDirection );
      GET_PACKET_U32(msg, color);
      msg->Get(&lKey);
...
void TFCMessagesHandler::RQFUNC_FromPreInGameToInGame(PACKET_FUNC_PROTOTYPE)
{
#ifndef _WIN32
   std::fprintf( stderr,
                 "[FromPreInGameToInGame] UDP req %s (preInGame=%d in_game=%d)\n",
                 user != NULL ? (LPCTSTR)user->GetAccount() : "?",
                 user != NULL && user->boPreInGame ? 1 : 0,
                 user != NULL && user->in_game ? 1 : 0 );
   std::fflush( stderr );
#endif
   if( user->boLockedOut || !user->CanPaidPlay() || !user->CanConnectGMOnly())
   {
   ...
   else
   {
      ...
      user->self->SendPlayerMessage( sending );
#if !defined( _WIN32 )
      /* deja in_game : ne pas bloquer UDP avec packet_inview_units (cf. GetNearItems async). */
      std::fprintf( stderr,
                    "[FromPreInGameToInGame] deja in_game pour %s — opcode 46 code=1 seulement\n",
                    (LPCTSTR)user->GetAccount() );
      std::fflush( stderr );
#else
      ...
      if( wlWorld != NULL )
      {
         const int read = wlWorld->packet_inview_units( user->self->GetWL(), sending, _DEFAULT_RANGE, user->self );
         if( read > 0 )
            user->self->SendPlayerMessage( sending );
      }
#endif
...
void TFCMessagesHandler::RQFUNC_SendChatterMessage(PACKET_FUNC_PROTOTYPE)
{
   ...
   // Correction de la faille des paroles sur cc sans �tre en jeu
   if ( user->in_game ) 
   {
      ChatterChannels &cChatter = CPlayerManager::GetChatter();
...
void TFCMessagesHandler::RQFUNC_GetNearItems( PACKET_FUNC_PROTOTYPE )
{
   ...
   {
#if !defined( _WIN32 )
      /* Comme Windows : inview meme en preInGame (teleport/mort) — pas de reponse vide. */
      {
         LPASYNC_PACKET_FUNC_PARAMS lpParams = new ASYNC_PACKET_FUNC_PARAMS;
         lpParams->user = user;
         lpParams->rqRequestID = rqRequestID;
         lpParams->msg = NULL;
         AsyncFuncQueue::GetMainQueue()->Call( AsyncRQFUNC_GetNearItems, lpParams );
         std::fprintf( stderr,
                       "[GetNearItems] enqueue async pour %s (preInGame=%d in_game=%d)\n",
                       (LPCTSTR)user->GetAccount(),
                       user->boPreInGame ? 1 : 0,
                       user->in_game ? 1 : 0 );
         std::fflush( stderr );
...
void TFCMessagesHandler::RQFUNC_NM_BuyAHItems(PACKET_FUNC_PROTOTYPE)
{
   ...
   else
   {
      ...
      GET_PACKET_U32(msg, dwIndex);
      msg->Get((char  *)&chBuy );
      GET_PACKET_U32(msg, dwPrix);
      GET_PACKET_U32(msg, dwPrixNMS);
      GET_PACKET_U32(msg, dwTS);
...
void TFCMessagesHandler::RQFUNC_NM_NMSGOLD_UtiliserPanier(PACKET_FUNC_PROTOTYPE)
{
   ...
   switch(dwFlag) 
   {
      case ciNMSGoldSGLv1:
      {
         ...
         {
            bFound = true;
            //oki rendu ici on peu passer a D�chu...
         ...
         if(user->GetNMSGoldToDLv1() > 0)
         {
            ...
            if(user->self->ViewFlag( __FLAG_NUMBER_OF_REMORTS ) ==5)
            {
               ...
               if(iErr == 0)
               {
                  ...
                  _LOG_ACHAT_NMS
                     LOG_ALWAYS,
                     "Player %s (%s) use PASSAGE � D�chu Lvl 1...",
                     (LPCTSTR)user->self->GetTrueName(),
                     (LPCTSTR)user->GetFullAccountName()
                     LOG_
...
/******************************************************************************/
{
    ...
    while( tlLocalUsers.QueryNext() && !boLocal )
	{
        ...
        TRACE( "\r\nIP %u vs %u, netmasked %u vs %u.", 
            NM_IPV4_ADDR(sockAddr.sin_addr),
            lpLocal->dwIP,
            NM_IPV4_ADDR(sockAddr.sin_addr) & lpLocal->dwNetmask,
            lpLocal->dwIP & lpLocal->dwNetmask
        );
        if( ( NM_IPV4_ADDR(sockAddr.sin_addr) & lpLocal->dwNetmask ) == ( lpLocal->dwIP & lpLocal->dwNetmask ) )
		{
            boLocal = TRUE;
        }
    }
...
{
   RQ_HEADER;
   if(user == NULL || user->self == NULL || !user->in_game)
      return;
...
#ifndef _WIN32
            std::fprintf( stderr,
                          "[Move] compte='%s' dir=%d pos %u,%u w=%u\n",
                          (LPCTSTR)user->GetAccount(),
                          static_cast<int>( rqRequestID ),
                          user->self->GetWL().X,
                          user->self->GetWL().Y,
                          user->self->GetWL().world );
            std::fflush( stderr );
...
#ifndef _WIN32
      std::fprintf( stderr,
                    "[PutPlayerInGame] ignore compte='%s' locked=%d paid=%d gm=%d\n",
                    (LPCTSTR)user->GetAccount(),
                    user->boLockedOut ? 1 : 0,
                    user->CanPaidPlay() ? 1 : 0,
                    user->CanConnectGMOnly() ? 1 : 0 );
#endif
      return;
   }

   const bool canEnter = user->registred || user->GetKeyCode() != 0;
   // if player isn't in game (for a change..!!! :))
   if(!user->in_game && !user->boPreInGame && canEnter)
   {
      if( user->UsePicklock(__FILE__, __LINE__) )
      {
         try
      ...
      {
         std::fprintf( stderr,
                       "[PutPlayerInGame] UsePicklock refuse compte '%s'\n",
                       (LPCTSTR)user->GetAccount() );
         TFCPacket busyReply;
         busyReply << (RQ_SIZE)RQ_PutPlayerInGame;
         busyReply << (char)7;
...
#ifndef _WIN32
      std::fprintf( stderr,
                    "[PutPlayerInGame] ignore compte='%s' in_game=%d preInGame=%d registred=%d key=%ld\n",
                    (LPCTSTR)user->GetAccount(),
                    user->in_game ? 1 : 0,
                    user->boPreInGame ? 1 : 0,
                    user->registred ? 1 : 0,
...
void TFCMessagesHandler::RQFUNC_RegisterAccount(PACKET_FUNC_PROTOTYPE)
{
...
#ifndef _WIN32
   std::fprintf( stderr, "[AUTH] RegisterAccount recu IP %s:%u\n",
                 inet_ntoa( sockAddrO.sin_addr ),
                 static_cast<unsigned>( ntohs( sockAddrO.sin_port ) ) );
   std::fflush( stderr );
#endif
   ...
   Players *existingUser = CPlayerManager::PeekPlayerAtEndpoint( sockAddrO );
   Players *lpReusePlayer = NULL;
   if( existingUser != NULL )
   {
      if( !existingUser->IsDeleteFlags()
          && existingUser->GetKeyCode() != 0 && !existingUser->in_game
          && ( existingUser->registred || existingUser->boPreInGame ) )
      {
#ifndef _WIN32
         std::fprintf( stderr,
                      "[AUTH] RegisterAccount: session deja auth IP/port — renvoi accord (char)0 + key "
                      "(registred=%d preInGame=%d)\n",
                      existingUser->registred ? 1 : 0,
                      existingUser->boPreInGame ? 1 : 0 );
#endif
         _LOG_DEBUG
            LOG_DEBUG_LVL3,
         ...
         TFCPacket sending;
         sending.SetPacketSeedID( msg->GetPacketSeedID() );
         sending << (short)RQ_RegisterAccount;
         sending << (char)0;
         sending << (CString)_DEFAULT_STR( 453 );
         sending << (long)existingUser->GetKeyCode();

         WorldPos wlPos = { -1, -1, -1 };
         CPacketManager::SendPacket( sending, sockAddrO, sockAddrI, -1, wlPos, FALSE );
         return;
      }

      if( !existingUser->in_game )
      {
         _LOG_DEBUG
            LOG_DEBUG_LVL3,
            "RegisterAccount: ghost session on IP/port — purge sync then re-auth."
            LOG_
#ifndef _WIN32
         std::fprintf( stderr,
                       "[AUTH] RegisterAccount: ghost IP/port — ForceReconnectPurge\n" );
         std::fflush( stderr );
#endif
         CPlayerManager::ForceReconnectPurge( sockAddrO );
      }
      else
      {
#ifndef _WIN32
         std::fprintf( stderr,
                       "[AUTH] RegisterAccount: in_game meme port — re-auth async (purge async)\n" );
#endif
         if( existingUser->IsDeleteFlags() || existingUser->in_game )
            lpReusePlayer = CPlayerManager::ResetFlaggedSessionForReauth( sockAddrO );
      }
   }
   ...
   if( !account.IsEmpty() )
   {
#ifndef _WIN32
      std::fprintf( stderr,
                    "[AUTH] RegisterAccount sync purge compte='%s' IP %s:%u\n",
                    (LPCTSTR)account,
                    inet_ntoa( sockAddrO.sin_addr ),
                    static_cast<unsigned>( ntohs( sockAddrO.sin_port ) ) );
      std::fflush( stderr );
      Players::DeleteOnlineUserSync( account );
      CPlayerManager::PurgeAccountSessionsExcept( account, sockAddrO );
#endif
      ...
      msg->Get((short *)(&lpStruct->usLangUsed)); // Ajout du syst�me multilingue.
      lpStruct->packetSeedID = msg->GetPacketSeedID();
      lpStruct->lpReusePlayer = lpReusePlayer;

      // Auth ODBC en thread async (apres purge sync des anciennes sessions).
...
static void AcceptVoluntaryExitGame( Players *user )
{
   if( user == NULL || user->self == NULL || user->IsDeleteFlags() )
      return;

   const bool wasInGame = user->in_game ? true : false;
   const bool wasPreInGame = user->boPreInGame ? true : false;
   const sockaddr_in exitAddr = user->IPaddrO;

   TFCPacket sending;
   sending << (RQ_SIZE)RQ_SafePlug;
   sending << (char)1;
   user->self->SendPlayerMessage( sending );
   ...
   if( wasInGame )
   {
      Broadcast::BCObjectRemoved( user->self->GetWL(), _DEFAULT_RANGE_REMOVE, user->self->GetID() );
   }
   ...
   user->SetKeyCode( 0 );
   user->dwExitDecompte = 0xFFFF;
   user->SetDeletePlayerFlags();
   user->ClearLoadingCount();

#ifndef _WIN32
   std::fprintf( stderr,
                 "[AUTH] ExitGame accepte compte='%s' IP %s:%u wasInGame=%d wasPreInGame=%d\n",
                 (LPCTSTR)user->GetAccount(),
                 inet_ntoa( exitAddr.sin_addr ),
                 static_cast<unsigned>( ntohs( exitAddr.sin_port ) ),
                 wasInGame ? 1 : 0,
                 wasPreInGame ? 1 : 0 );
   std::fflush( stderr );
#endif
...
void TFCMessagesHandler::RQFUNC_SafePlug( PACKET_FUNC_PROTOTYPE )
{
   RQ_HEADER;

#ifndef _WIN32
   std::fprintf( stderr, "[AUTH] SafePlug (123) recu compte='%s' in_game=%d preInGame=%d IP %s:%u\n",
                 user != NULL ? (LPCTSTR)user->GetAccount() : "?",
                 user != NULL && user->in_game ? 1 : 0,
                 user != NULL && user->boPreInGame ? 1 : 0,
                 inet_ntoa( sockAddrO.sin_addr ),
   ...
   try
   {
      ...
      if( status == 0 && user->self != NULL )
      {
         if( user->self->GetUnderBlockMap() == __SAFE_HAVEN
             || user->self->GetUnderBlockMap() == __INDOOR_SAFE_HAVEN
             || user->IsGod() )
         {
            user->dwExitDecompte = 0;
...
void TFCMessagesHandler::RQFUNC_ExitGame(PACKET_FUNC_PROTOTYPE)
{
...
#ifndef _WIN32
   std::fprintf( stderr, "[AUTH] ExitGame recu compte='%s' in_game=%d preInGame=%d IP %s:%u\n",
                 user != NULL ? (LPCTSTR)user->GetAccount() : "?",
                 user != NULL && user->in_game ? 1 : 0,
                 user != NULL && user->boPreInGame ? 1 : 0,
                 inet_ntoa( sockAddrO.sin_addr ),
   ...
   if( user == NULL || user->IsDeleteFlags() )
      return;

   if( !user->in_game && !user->boPreInGame && !user->registred && user->GetKeyCode() == 0 )
   {
#ifndef _WIN32
      std::fprintf( stderr,
                    "[AUTH] ExitGame ignore compte='%s' (deja deconnecte) IP %s:%u\n",
                    (LPCTSTR)user->GetAccount(),
                    inet_ntoa( sockAddrO.sin_addr ),
                    static_cast<unsigned>( ntohs( sockAddrO.sin_port ) ) );
#endif
   ...
   {
      const bool bSafe = user->self->GetUnderBlockMap() == __SAFE_HAVEN
                         || user->self->GetUnderBlockMap() == __INDOOR_SAFE_HAVEN
                         || user->IsGod();

      if( bSafe )
         bAccept = true;
      else if( theApp.dwAntiplugSystem != 1 )
         bAccept = true;
      else if( user->dwExitDecompte == 0 )
         bAccept = true;
      else
      {
         const unsigned long lNow = (unsigned long)time( NULL );
         const unsigned long lLast = user->ulLastPacketTime;
         if( lNow >= lLast && ( lNow - lLast ) >= 13 )
            bAccept = true;
         else if( user->dwExitDecompte == 0xFFFF )
         {
   ...
   if( bAccept )
      AcceptVoluntaryExitGame( user );
#ifndef _WIN32
   else
   {
      std::fprintf( stderr,
                    "[AUTH] ExitGame refuse compte='%s' (antiplug, attendre ~13 s inactivite)\n",
                    (LPCTSTR)user->GetAccount() );
   }
#endif
...
void TFCMessagesHandler::RQFUNC_UseObject2(PACKET_FUNC_PROTOTYPE)
{
   ...
   if(wl)
   {
      ...
      msg->Get((long  *)(&itemID));
      GET_PACKET_U32(msg, dwTargetID);
      msg->Get((unsigned char *)(&chAttack));
      msg->Get((unsigned char *)(&chPVP));
...
void TFCMessagesHandler::RQFUNC_EnterChatterChannel(PACKET_FUNC_PROTOTYPE)
{
   ...
   GET_STRING( szChannel );
   std::unique_ptr<BYTE[]> cAutoDeleteChannel(szChannel);
   GET_STRING( szPassword );                
   std::unique_ptr<BYTE[]> cAutoDeletePassword(szPassword);
...
void TFCMessagesHandler::RQFUNC_UseItemByAppearance( PACKET_FUNC_PROTOTYPE )
{
   ...
   // On v�rifie si le pointeur est null (cela corrige certain crash serveur)
   if( user->self == NULL )
   {
      _LOG_DEBUG
         LOG_DEBUG_LVL1,
...
void TFCMessagesHandler::RQFUNC_NM_NMSGOLD_AchatOpt2(PACKET_FUNC_PROTOTYPE)
{
   ...
   for(int i=0;i<nbrItems;i++)
   {
      //send name + Desc + prix
      CString csRealName;
      csRealName.Format("%s [Qt�. %d]",theApp.m_aAchatOpt2[i].strName,theApp.m_aAchatOpt2[i].iNbrItem);
      sending << csRealName;
