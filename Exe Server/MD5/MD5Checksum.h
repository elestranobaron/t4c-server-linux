...
/* Portage Linux 64-bit : MD5 exige des mots de 32 bits ; le type historique (long non signe)
 * fait 8 octets sur LP64 (assert sizeof(x)==4 fatal dans RotateLeft). On force uint32_t. */
typedef std::uint32_t md5_u32;

class CMD5Checksum  
{
public:
	//interface functions for the RSA MD5 calculation
	static char* GetMD5(unsigned char* pBuf, unsigned int nLength);
...
protected:
	...
	void Update(unsigned char* Input, md5_u32 nInputLen);
	char* Final();
	inline md5_u32 RotateLeft(md5_u32 x, int n);
	inline void FF( md5_u32& A, md5_u32 B, md5_u32 C, md5_u32 D, md5_u32 X, md5_u32 S, md5_u32 T);
	inline void GG( md5_u32& A, md5_u32 B, md5_u32 C, md5_u32 D, md5_u32 X, md5_u32 S, md5_u32 T);
	inline void HH( md5_u32& A, md5_u32 B, md5_u32 C, md5_u32 D, md5_u32 X, md5_u32 S, md5_u32 T);
	inline void II( md5_u32& A, md5_u32 B, md5_u32 C, md5_u32 D, md5_u32 X, md5_u32 S, md5_u32 T);

	//utility functions
	void DWordToByte(unsigned char* Output, md5_u32* Input, unsigned int nLength);
	void ByteToDWord(md5_u32* Output, unsigned char* Input, unsigned int nLength);

private:
	unsigned char  m_lpszBuffer[64];		//input buffer
	md5_u32 m_nCount[2];			//number of bits, modulo 2^64 (lsb first)
	md5_u32 m_lMD5[4];			//MD5 checksum
};
