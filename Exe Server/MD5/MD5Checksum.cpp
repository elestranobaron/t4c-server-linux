...
DETAILS:		private
DESCRIPTION:	Rotates the bits in a 32 bit md5_u32 left by a specified amount
RETURNS:		The rotated md5_u32 
ARGUMENTS:		md5_u32 x : the value to be rotated
				int n   : the number of bits to rotate by
*****************************************************************************************/
md5_u32 CMD5Checksum::RotateLeft(md5_u32 x, int n)
{
	//check that md5_u32 is 4 unsigned chars long - true in Visual C++ 6 and 32 bit Windows
	_ASSERT( sizeof(x) == 4 );
...
RETURNS:		none
ARGUMENTS:		md5_u32 &A, B, C, D : Current (partial) checksum
				md5_u32 X           : Input data
				md5_u32 S			  : MD5_SXX Transformation constant
				md5_u32 T			  :	MD5_TXX Transformation constant
NOTES:			None
*****************************************************************************************/
void CMD5Checksum::FF( md5_u32& A, md5_u32 B, md5_u32 C, md5_u32 D, md5_u32 X, md5_u32 S, md5_u32 T)
{
	md5_u32 F = (B & C) | (~B & D);
	A += F + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void CMD5Checksum::GG( md5_u32& A, md5_u32 B, md5_u32 C, md5_u32 D, md5_u32 X, md5_u32 S, md5_u32 T)
{
	md5_u32 G = (B & D) | (C & ~D);
	A += G + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void CMD5Checksum::HH( md5_u32& A, md5_u32 B, md5_u32 C, md5_u32 D, md5_u32 X, md5_u32 S, md5_u32 T)
{
	md5_u32 H = (B ^ C ^ D);
	A += H + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void CMD5Checksum::II( md5_u32& A, md5_u32 B, md5_u32 C, md5_u32 D, md5_u32 X, md5_u32 S, md5_u32 T)
{
	md5_u32 I = (C ^ (B | ~D));
	A += I + X + T;
	A = RotateLeft(A, S);
...
RETURNS:		void
ARGUMENTS:		md5_u32* Output : the 32 bit (md5_u32) destination array 
...
NOTES:			Four unsigned charS from the input array are transferred to each md5_u32 entry
				of the output array. The first unsigned char is transferred to the bits (0-7) 
				of the output md5_u32, the second unsigned char to bits 8-15 etc. 
...
*****************************************************************************************/
void CMD5Checksum::ByteToDWord(md5_u32* Output, unsigned char* Input, unsigned int nLength)
{
	//entry invariants
	_ASSERT( nLength % 4 == 0 );
	...
	for ( ; j < nLength; i++, j += 4)
	{
		Output[i] = (md5_u32)Input[j]			| 
					(md5_u32)Input[j+1] << 8	| 
					(md5_u32)Input[j+2] << 16 | 
					(md5_u32)Input[j+3] << 24;
	}
}
...
void CMD5Checksum::Transform(unsigned char Block[64])
{
	...
	//copy unsigned charS from input 'Block' to an array of md5_u32S 'X'
	md5_u32 X[16];
...
ARGUMENTS:		unsigned char* Output  : the 8 bit destination array 
				md5_u32* Input  : the 32 bit source array
				unsigned int nLength  : the number of 8 bit data items in the source array
NOTES:			One md5_u32 from the input array is transferred into four unsigned charS 
				in the output array. The first (0-7) bits of the first md5_u32 are 
				...
				so that there is a perfect fit of 8 bit unsigned charS into the 32 bit md5_u32s.
*****************************************************************************************/
void CMD5Checksum::DWordToByte(unsigned char* Output, md5_u32* Input, unsigned int nLength )
{
	//entry invariants
	_ASSERT( nLength % 4 == 0 );
...
char * CMD5Checksum::Final()
{
   ...
   memset(pstrReturn ,0x00,64);
	for ( int i=0; i < nMD5Size; i++) 
	{
      ...
      {
         sprintf_s(Str,20,"0%x",lpszMD5[i]);
		}
		else 
      {
         sprintf_s(Str,20,"%x",lpszMD5[i]);
		}

		_ASSERT( strlen(Str) == 2 );
      strcat_s(pstrReturn,64,Str);
	}
...
*****************************************************************************************/
void CMD5Checksum::Update( unsigned char* Input,	md5_u32 nInputLen )
{
