    ...
    while( boCheck ){
        ...
        for( z = watchList.begin(); z != watchList.end(); z++ ){
            ...
            if( watch.dwTimeout <= dwCurrentTime ){
                ...
                std::fflush(stderr);
                /* Si c'est le thread de reception UDP : dumper opcode + backtrace pour
                 * localiser le handler synchrone qui bloque. */
                if( std::strstr( (LPCTSTR)watch.csThreadName, "UDPReceivePacketThread" ) != NULL )
                {
                    extern void DbgDumpReceiveStall();
                    DbgDumpReceiveStall();
                }
