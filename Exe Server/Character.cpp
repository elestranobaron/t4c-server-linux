...
void Character::LogStartupItems( void )
{
#ifndef _WIN32
   vStartupItems.Lock();
   for( vector<string>::iterator it = vStartupItems.begin(); it != vStartupItems.end(); ++it )
   {
      const WORD wID = Unit::GetIDFromName( (*it).c_str(), U_OBJECT, TRUE );
      std::fprintf( stderr, "[BOOT] StartupItem '%s' -> summon ID %u%s\n",
                    (*it).c_str(), static_cast<unsigned>( wID ),
                    wID == 0 ? " (INVALIDE — corriger T4CServer.ini)" : "" );
   }
   vStartupItems.Unlock();
   std::fflush( stderr );
#endif
...
{	
   ...
   else
   {
      ...
      // /!\ Si on passe de l'ancienne ? la nouvelle     *
      //     il faut passer le script de mise ? jour     *
      //     sur la base de donn?e.                      *
      ///////////////////////////////////////////////////////
      // Constante d'XP suppl?mentaire ? Ajouter au lvl 191+
      ...
      for( nLevel = 1; nLevel < MAX_LEVEL_XP; nLevel++ )
      {
         ...
         else // lvl > 191
         {
            ...
            // => Xp necessaire pour aller du niveau pr?c?dent au suivant + constante
               ...
               {																		
                  ...
                  lpSkill->SetSkillID  ( dwSkillID );
                     ...
                     if(dwResetSpellID == 0)
                     {
                        ...
                        else if(dwSkillID == 10154)//r?sistance au feu
                        {
                           wNbSkillPnts+=19;
                           SetGold(GetGold()+69617,false);
                        }
                        else if(dwSkillID == 10170)// r?sistance ? la glace
                        {
                           wNbSkillPnts+=20;
                           SetGold(GetGold()+79028,false);
                        }
                        else if(dwSkillID == 10150)//enchev?trement
                        {
                           wNbSkillPnts+=15;
                     ...
                     else if(dwResetSpellID == 1)
                     {
                        ...
                        else if(dwSkillID == 10852)//Racines v?n?neuses
                        {
                           wNbSkillPnts+=50;
...
{	
   ...
   double  strike3 =  blow->Strike;
	...
	if( ((Character*)this)->GetGodFlags() & GOD_DEVELOPPER ) {
   		...
   		CString csMessage;
		csMessage.Format( "[\"attack\"] degats avant : %lf, d?gat naturels : %lf, d?gats finaux, %lf ", strike1, strike2, strike3);
   ...
   if( boFound == false )
   {
      ...
      {    
         ...
         while( tlObjSpillList.QueryNext() ){
            ...
            wlFoundPos.X = wlPlayerPos.X; break;//BLBLBL ajout? 0 ici (et pass? ? 8 choix)
...
/******************************************************************************/
{
   ...
   if( user != NULL )
   {
      ...
      {
         /* Memorise les PV avant le coup (utilises a la resurrection si ce coup tue). */
         dwHPBeforeLastBlow = ( HP > 0 ) ? (DWORD)HP : 1;
         HP -= (int)blow->Strike;	
...
/******************************************************************************/
{
   if( GetPlayer() == NULL || GetPlayer()->IsDeleteFlags() )
      return 1;
...
(
 const CString &csMessage, // The message to be sent.
 ...
 /******************************************************************************/
{
   ...
   // pl peut etre NULL : unite non-joueur, ou Character d'un joueur dont le lien
   // Players n'est pas (ou plus) etabli (ex. apres reconnexion). Sans cette garde,
   // le deref crashe le serveur (regression vs 1.68 qui teste toujours pl != NULL).
   if( pl != NULL && ( pl->GetGodFlags() & GOD_TRUE_INVISIBILITY ) )
   {
...
/******************************************************************************/
{
   ...
   for( i = 0; i < Limit; i++ )
   {
      ...
      else
      {
         ...
         Players *pl = reinterpret_cast< Players * >( GetPlayer() );
         if( pl != NULL && ( pl->GetGodFlags() & GOD_DEVELOPPER ) )
         {
...
void Character::DeathNMS(Unit *WhoHit)
{
   ...
   if(theApp.m_dwPVPSyetem2Actif == 1 && bDropPVPChange)
   {
      ...
      strCC = "PVP";
      ChatterChannels &cChatter = CPlayerManager::GetChatter();
      cChatter.TalkSystem( "Syst?me", (char*)strCC.GetBuffer(0), (char*)strTmp.GetBuffer(0) );
   }
...
/******************************************************************************
 * Rammener ? la vie (cr?e pour NMS 1.7)
 ******************************************************************************/
void Character::NMResurect(BOOL bForceResurect)
{
   ...
   /* T4C 1.25 : on ressuscite avec les PV qu'on avait AVANT le coup fatal. */
   {
      DWORD dwResHP = dwHPBeforeLastBlow;
      if( dwResHP == 0 || dwResHP > GetMaxHP() )
         dwResHP = GetMaxHP();
      SetHP( dwResHP, true );
   }
...
void Character::StartMove(short shDirection)
{
   ...
   if( newExhaust.move <= TFCMAIN::GetRound() || ( newExhaust.boWalking && newExhaust.move - TFCMAIN::GetRound() <= MoveExhaust ))
   {
      ...
      {				
         ...
         {
            TFCPacket moveAck;
            moveAck << (RQ_SIZE)__EVENT_OBJECT_MOVED;
            moveAck << (short)GetWL().X;
            moveAck << (short)GetWL().Y;
            PacketUnitInformation( moveAck );
            SendPlayerMessage( moveAck );
         }
      }
      else if( world != NULL && searchway >= 0 )
      {
         /* Pas de changement de tuile (mur, collision) — renvoyer quand meme la position
          * courante pour debloquer le client (bCanMove / canMove_). */
         TFCPacket moveAck;
         moveAck << (RQ_SIZE)__EVENT_OBJECT_MOVED;
         moveAck << (short)GetWL().X;
         moveAck << (short)GetWL().Y;
         ...
         if( PreTranslateInGameMessage( lpMessageSent ) )
         {
            ...
            if( boProcess )
            {
               ...
               if( target != NULL )
               {
                  ...
                  StopAutoCombat();
#ifndef _WIN32
                  std::fprintf( stderr, "[DirectTalk] '%s' -> '%s' (type=%u, doing=%u, baseRef=%u) msg='%.60s'\n",
                                (LPCTSTR)GetTrueName(), (LPCTSTR)target->GetName( _DEFAULT_LNG ),
                                (unsigned)target->GetType(), (unsigned)target->IsDoing(),
                                (unsigned)target->GetStaticReference(),
                                (const char *)lpMessageSent );
                  std::fflush( stderr );
#endif
...
void Character::StartAsyncFromPregameToGame()
{
   ...
   if( GetPlayer()->boPreInGame && !GetPlayer()->in_game )
   {
      ...
      if( !result )
      {
         ...
         GetPlayer()->self->StartViewBackpack2(true,0);
         ...
         WorldMap *wlWorldEntry = TFCMAIN::GetWorld( GetWL().world );
         if( wlWorldEntry != NULL )
         {
            TFCPacket inviewPkt;
            const int inviewCount =
               wlWorldEntry->packet_inview_units( GetWL(), inviewPkt, _DEFAULT_RANGE, GetPlayer()->self );
            if( inviewCount > 0 )
            {
               SendPlayerMessage( inviewPkt );
#ifndef _WIN32
               std::fprintf( stderr,
                             "[FromPreInGameToInGame] inview push %d unite(s) @ %u,%u w=%u pour %s\n",
                             inviewCount,
                             static_cast<unsigned>( GetWL().X ),
                             static_cast<unsigned>( GetWL().Y ),
                             static_cast<unsigned>( GetWL().world ),
                             GetPlayer() != NULL ? (LPCTSTR)GetPlayer()->GetAccount() : "?" );
               std::fflush( stderr );
#endif
            }
#ifndef _WIN32
            ...
            {
               std::fprintf( stderr,
                             "[FromPreInGameToInGame] inview vide @ %u,%u w=%u pour %s\n",
                             static_cast<unsigned>( GetWL().X ),
                             static_cast<unsigned>( GetWL().Y ),
                             static_cast<unsigned>( GetWL().world ),
                             GetPlayer() != NULL ? (LPCTSTR)GetPlayer()->GetAccount() : "?" );
      ...
      {
         std::fprintf( stderr,
                       "[FromPreInGameToInGame] opcode 46 envoye code=%d mais GetWorld(%u) NULL pour %s\n",
