...
/******************************************************************************/
{
   ...
   while( 1 )
   {
      ...
      InterlockedExchange( reinterpret_cast< long * >( &g_dwRound ), static_cast< long >( dummyRound ) );
...
#ifndef _WIN32
	{
		/* Build monolithique Linux : DllMain (qui enregistrait MonsterStatSetup via
		 * RegisterUnitStartupFunction) ne tourne jamais. Sans cet appel, aucun template
		 * NPC n'est rempli et CreateNPC_<region>() n'est jamais invoque : Mithrand,
		 * Araknor, les marchands etc. n'apparaissent pas en jeu. */
		extern void MonsterStatSetup(void);
		TFCMAIN::RegisterUnitStartupFunction( MonsterStatSetup );
		std::fprintf( stderr, "[BOOT] MonsterStatSetup enregistre (NPC DLL monolithiques)\n" );
		std::fflush( stderr );
	}
#endif
...
#ifndef _WIN32
    dwVersion = regKey.GetProfileInt( "Version", 1720 );
#else
