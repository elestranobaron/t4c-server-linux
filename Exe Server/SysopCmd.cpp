...
/******************************************************************************/
{
	...
	// Replace the trailing \0 with a ? (char 216) to have a last token.
...
/******************************************************************************/
{
	...
	csCommand.TrimLeft();
	//csCommand.MakeUpper();
	csCommand += '?'; // Add command terminator (char 216)
   ...
   COMMAND( "SUMMON ITEM $", GOD_CAN_SUMMON_ITEMS )
	...
	COMMAND( "SET $'S $ TO $", GOD_CAN_EDIT_USER )	
	{
		...
		COMPONENT( "LEVEL", GOD_CAN_EDIT_USER_XP_LEVEL, 1 )
         ...
         CString csCheckName = csName.SpanIncluding(_T("1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ -??????????????"));
...
/******************************************************************************/
{
	...
	/******************************************************************************/
	{
		std::unique_ptr<SetNameData> nameData(reinterpret_cast<SetNameData *>(lpData));
