...
#else
	/* Linux monolithique : les PNJ (Mithrand, Araknor, marchands…) s'enregistrent via
	 * des constructeurs statiques AVANT ce point. Le tableau statique est deja zero-
	 * initialise par le loader ; remettre a NULL ici effacerait toutes ces inscriptions
	 * (sous Windows les DLL s'enregistraient apres, donc le wipe etait sans effet). */
	int iRegistered = 0;
	for( int i = 0; i < 65536; i++ ){
		if( lpMessagesProc[ i ] != NULL )
			iRegistered++;
	}
	std::fprintf( stderr, "[BOOT] InitializeMessagesProcs: %d unite(s) deja enregistree(s) conservee(s)\n",
	              iRegistered );
	std::fflush( stderr );
#endif
}
...
#ifndef _WIN32
        if( MessageID == MSG_OnTalk ){
            std::fprintf( stderr, "[NPCTalk] PAS de handler OnTalk pour '%s' (type=%u baseRef=%u)\n",
                          (LPCTSTR)GetName( _DEFAULT_LNG ), (unsigned)GetType(),
                          (unsigned)BaseReferenceID );
            std::fflush( stderr );
        }
#endif
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    if( GetType() != U_PC )
    {
        std::fprintf( stderr, "[NPCTalk] '%s' dit: %.80s\n",
                      (LPCTSTR)GetName( _DEFAULT_LNG ), (const char *)Message );
        std::fflush( stderr );
    }
#endif
...
WORD Unit::GetINT( void ){
	int nStat = StatINT;
	nStat += QueryBoost( STAT_INT );
	nStat = nStat < 0 ? 0 : nStat;
	return (WORD)nStat;
}

WORD Unit::GetEND( void ){
	int nStat = StatEND;
	nStat += QueryBoost( STAT_END );
	nStat = nStat < 0 ? 0 : nStat;
	return (WORD)nStat;
}

WORD Unit::GetAGI( void ){
	int nStat = StatAGI;
	nStat += QueryBoost( STAT_AGI );
	nStat = nStat < 0 ? 0 : nStat;
	return (WORD)nStat;
}

WORD Unit::GetWIS( void ){
	int nStat = StatWIS;
	nStat += QueryBoost( STAT_WIS );
	nStat = nStat < 0 ? 0 : nStat;
	return (WORD)nStat;
}

WORD Unit::GetATTACK( void ){
	int nStat = StatATTACK;
	nStat += QueryBoost( STAT_ATTACK );
	nStat = nStat < 0 ? 0 : nStat;
	return (WORD)nStat;
}

WORD Unit::GetDODGE( void ){
	int nStat = StatDODGE;
	nStat += QueryBoost( STAT_DODGE );
	nStat = nStat < 0 ? 0 : nStat;
	return (WORD)nStat;
}

WORD Unit::GetLCK( void ){
	int nStat = StatLCK;
	nStat += QueryBoost( STAT_LCK );
	nStat = nStat < 0 ? 0 : nStat;
	return (WORD)nStat;
}
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    {
        Character *ch = static_cast< Character * >( unit );
        // Une unite PC peut subsister en jeu sans Players associe (session
        // orpheline en cours de teardown apres logout/reconnexion). Garder le
        // deref NULL pour eviter le SIGSEGV dans GetNearItems / packet_inview_units.
        Players *pl = ch->GetPlayer();
