...
/******************************************************************************/
{
	InitTalk 
		...
		Begin
			""
			IF (IsInRange(3))		 
				;BYTE bPositionWorld = BYTE(target->ViewFlag(__ARENA_RETURN_LOC) & 0xFF);
				WORD wPositionY = WORD((target->ViewFlag(__ARENA_RETURN_LOC) & 0xFFF00) >> 8);
				WORD wPositionX = WORD((target->ViewFlag(__ARENA_RETURN_LOC) & 0xFFF00000) >> 20);
