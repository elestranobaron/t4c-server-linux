...
/******************************************************************************/
{
	...
	SET_NPC_NAME( "[10970]A wealthy-looking man" );  
	   ...
	   npc.InitialPos.X     = 0;//345; 
	   npc.InitialPos.Y     = 0;//490;
...
/******************************************************************************/
{
	...
	CONSTANT GoToBattleGround = 5; 
		...
		YES(WantToFight)
			IF ((ACK_COLOSSEUM == 0) || (CheckFlag(__FLAG_NUMBER_OF_REMORTS) >= 1) || (CheckFlag(__FLAG_NMS_DECHU) > 0))
				Conversation
		...
		;} else if( ( msg.Find( INTL( 10737, "COLOSSEUM") ) != -1 ) && YesNo == Gladiator) {;YesNo = 0;
			IF (ACK_COLOSSEUM == 0) 		
				...
				BREAK 
			ELSEIF (CheckFlag(__FLAG_NUMBER_OF_REMORTS) >= 1 || (CheckFlag(__FLAG_NMS_DECHU) > 0)) // XP Colosseum
				GiveFlag(__FLAG_USER_HAS_ENTERED_COLOSSEUM, 1)
				GiveFlag(__FLAG_ARENA_LEVEL, 0)
				GiveFlag(__FLAG_USER_LEVEL_SLICE, 0)
				Conversation
