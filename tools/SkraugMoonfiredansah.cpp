...
extern NPCstructure::NPC SkraugMoonfiredansahNPC;

void SkraugMoonfiredansah::Create(){
   npc = SkraugMoonfiredansahNPC;
   SET_NPC_NAME( "[10923]Skraug moonfiredansah" );
   npc.InitialPos.X = 0; 
   npc.InitialPos.Y = 0; 
	npc.InitialPos.world = 0;
}

void SkraugMoonfiredansah::OnAttack( UNIT_FUNC_PROTOTYPE )
{  
	switch(rnd(0, 40))	 
	{		
...
void SkraugMoonfiredansah::OnAttacked( UNIT_FUNC_PROTOTYPE )
{ 
	switch(rnd(0, 40))	 
	{
...
void SkraugMoonfiredansah::OnDeath( UNIT_FUNC_PROTOTYPE )
{
	switch(rnd(0, 40))	 
	{	
