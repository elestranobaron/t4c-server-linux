...
Add Profession,Is Dead player,NMCombat Mode, Crime and Honor  by NightMNare (27/06/2009)
/******************************************************************************/
//      File Name: Unit.h
//      Project:   TFC Server
...
class __declspec( dllexport ) BaseReferenceMessages{
...
const BYTE U_HIVE	   = 4;
const BYTE U_MINIONS = 5;

#define U_PC_COLOR		   CL_RED;
#define U_PCRP_COLOR	      CL_BLUE_2;
#define U_GOD_COLOR		   CL_WHITE;
#define U_NPC_COLOR		   CL_BLUE_2;
#define U_OBJECT_COLOR	   CL_GREEN;
#define U_OBJECT_COLORYOUR	CL_YELLOW;
...
public:
    ...
    inline SkillID GetSkillID();

    inline SkillPnt GetSkillPnts( Unit *lpUnit );

    inline SkillPnt GetTrueSkillPnts( void );

    inline void SetSkillPnts( SkillPnt nPnt );

    inline void SetSkillID( SkillID nID );
...
typedef struct _USER_PROFESSION_F
{
   USHORT ushID;
	
} USER_PROFESSION_F, *LPUSER_PROFESSION_F;



//////////////////////////////////////////////////////////////////////////////////////////
// This class handles units!
class __declspec(dllexport) Unit : public DynamicFlags, public CLock
{	
public:	
   ...
   static UINT GetNbrUnitMap();
	...
	virtual void SetXP(__int64 xp,bool bforce=false);
	...
	virtual void Do(BYTE behavior,CString strFromWho = "");
	...
	virtual BOOL Teleport(WorldPos where, BYTE TeleportVisualEffect,BOOL bByPassAreneBlock = FALSE);
	...
	virtual void CheckIFLevelUP();	
	virtual BOOL UseSkillPnts(WORD wQuantity);
	virtual BOOL UseStatPnts(WORD wQuantity);
   ...
   virtual void SendPlayerXP(bool bForceUpdate);

	virtual BOOL UseSkill(int nID, Unit *target, LPVOID lpValueOUT = NULL );

	virtual BOOL UseSkill(int nID, WorldPos wlTarget);

	virtual void Talk( LPCTSTR Message, DWORD dwColor = CL_CYAN, Unit *TalkTo = NULL );
	...
	virtual double GetParryPC( void );
	...
	inline void RemoveEffect( DWORD dwEffect,  bool bBroadcast = true);
   ...
   inline void DispellEffects(Unit *caster, Unit *target, DWORD dwEffectID);
   ...
   inline void SendGlobalEffectMessage( BYTE bMsgType, LPVOID lpMsgData, Unit *medium, Unit *target );
    ...
    inline void RemoveAllBoost( void );
   ...
   virtual BOOL CanChangeTargetAA( void );
   virtual void SetChangeTargetAA( BOOL boChangeTargetAA );

   virtual DWORD GetFriendlyID( void );
   virtual void SetFriendlyID( DWORD dwID );

   virtual char GetNMCombatMode();

   virtual DWORD GetImmobilizationCycle(){return m_dwImmobilizationCycle;};
   virtual void  SetImmobilizationCycle(DWORD dwVal){m_dwImmobilizationCycle = dwVal;};

   virtual DWORD GetPrimalScreamCycle(){return m_dwPrimalScreamCycle;};
   virtual void  SetPrimalScreamCycle(DWORD dwVal){m_dwPrimalScreamCycle = dwVal;};
   ...
   BYTE GetUnderBlockMap( void );
    ...
    virtual void SetForceAttack( BOOL boFA );
    virtual BOOL IsForceAttack( void );

    virtual void SendPrivateMessage( const CString &csMessage, Unit *lpuUnit, DWORD dwColor = CL_CYAN );

    virtual void PacketUnitInformation( TFCPacket &sending );

    void SendSystemMessage( CString csText, DWORD dwColor = CL_BLUE_LIGHT ){
       const int RQ_ServerMessage = 63;
       TFCPacket sending;
	...
	void SendInfoMessage( CString csText, DWORD color = CL_BLUE_LIGHT, DWORD type = 0 )
   {
      const int RQ_InfoMessage = 102;
      TFCPacket sending;
    ...
    virtual int  GetCrime( void ){ return nCrime; };
    virtual void SetCrime( int nInt );

    virtual int  GetHonor( void ){ return nHonor; };
    virtual void SetHonor( int nInt );

    // New Luck attribute should use the flags interface as well
    virtual  WORD GetLCK( void );
    virtual  WORD GetTrueLCK( void );
    virtual  void SetLCK  ( WORD bStat );
    ...
    virtual void VaporizeUnit( bool bLog = true ) = 0;
   ...
   void  AddHitUnit(DWORD dwID,Unit *pUnit);
   void  DelAllHitUnit();
   void  ValidHitTime();
   void  GetUnitVector(std::vector <Unit*> &vUnit);

   DWORD GetLockedID(){return m_dwLockedID;};
   void  SetLockedID(DWORD dwVal){m_dwLockedID = dwVal;};
...
private:
   ...
   static CLock   csUnitMap;

   std::map<DWORD,HITData*> m_HitList;
   CLock   m_csUnitMap;
...
//	CString  Name;					// String defining the name of the unit
   ...
   DWORD m_dwImmobilizationCycle;
   DWORD m_dwPrimalScreamCycle;
    ...
    DWORD effectStackLevelDispell;
    DWORD effectStackLevelQuery;
