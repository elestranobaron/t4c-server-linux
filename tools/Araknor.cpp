...
IF(CheckFlag(__FLAG_NUMBER_OF_NECROMANTIC_SCROLLS_USER_WANTS) != 0)
	IF(CurrentRound < DWORD(CheckFlag(__FLAG_TIMER_ARAKNOR_STARTED_MAKING_SCROLLS)))
		IF(CurrentRound > 6000 * DWORD(CheckFlag(__FLAG_NUMBER_OF_NECROMANTIC_SCROLLS_USER_WANTS)))
		...
		ELSE
			...
			BREAK
		ENDIF
	ELSEIF(CurrentRound >= DWORD(CheckFlag(__FLAG_TIMER_ARAKNOR_STARTED_MAKING_SCROLLS)) + 6000 * DWORD(CheckFlag(__FLAG_NUMBER_OF_NECROMANTIC_SCROLLS_USER_WANTS)))
		IF(Gold >= 10000 * CheckFlag(__FLAG_NUMBER_OF_NECROMANTIC_SCROLLS_USER_WANTS))
			FOR(0, CheckFlag(__FLAG_NUMBER_OF_NECROMANTIC_SCROLLS_USER_WANTS))
				TakeGold(10000)
				GiveItem(__OBJ_NECROMANTIC_SCROLL)
...
IF (CheckItem (__OBJ_STONE_OF_LIFE) > 0)
   ...
   IF (CheckUnitFlag (StoneOfLife, __ITEM_QUEST_STONE_OF_LIFE)  == (CheckGlobalFlag (__QUEST_STONE_OF_LIFE)) && (DWORD(CheckGlobalFlag (__QUEST_STONE_OF_LIFE)) > CurrentRound ) )
      Conversation
...
IF ((CheckItem(__OBJ_JARKO_SPELLBOOK) >= 1) && (CheckFlag(__FLAG_KILLED_JARKO) >= 1))
   IF (DWORD(CheckNPCFlag (__JARKO_SPELLBOOK_TO_ARAKNOR)) > CurrentRound)
      Conversation      
