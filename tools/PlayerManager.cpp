      ...
      try
      {
         ...
         try
         {
            ...
            while( tlPlayersToDelete.QueryNext() )
            {
               ...
               if( wlWorld && lpPlayer->in_game) //CV_VALID devrait etre OK de valider le INGAME...
               {
                  lpPlayer->self->DestroyMinion();
                  wlWorld->remove_world_unit( lpPlayer->self->GetWL(), lpPlayer->self->GetID());
                  Broadcast::BCObjectRemoved( lpPlayer->self->GetWL(), _DEFAULT_RANGE_REMOVE,lpPlayer->self->GetID()); //quand player delete
               }
               lpPlayer->self->Unlock();
...
Players *CPlayerManager::GetPlayerResourceFct( sockaddr_in sockAddr )// The address of the player to fetch.
{
   ...
   // for too much time.
   const int  maxLoopCount = 50; //CV: A VALIDER initial 100 , before 500, now 50
   int    loopCount = 0;
