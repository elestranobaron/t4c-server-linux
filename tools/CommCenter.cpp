...
#else						//modif annul�e, apparement �a provoque des crashs dans le thread.
...
void CCommCenter::SendPacket(sockaddr_in sockAddr,LPBYTE lpBuffer,int nBufferSize,DWORD dwAckDelay,DWORD dwMaxAck)
{
   ...
   if(nBufferSize <= MAX_DATA_SIZE) //None splitted packet... Size of MAX+X_DATA_SIZE is 512 octets
   {
       ...
       //pPacket->dwTimeout    = 0xFFFFFFFF;//BLBLBL c'est d�bile, j'initialise avec un vrai temps plutot :
       ...
       if (dwMaxAck != 0) {  
	      ...
	      if (pPacket->dwAckDelay<=0){//BLBLBL 13/12/2010, on teste voir si par hasard on ne donnerait pas un d�lai de 0 
   ...
   else
   {
      ...
      for( int i = 0; i < numberOfPieces; ++i )
      {
         ...
         pPacket->sockAddr     = sockAddr;
         pPacket->dwAckDelay   = dwAckDelay;
         pPacket->dwTimeout    = GetRunTime() + pPacket->dwAckDelay;//04/12/2010 BLBLBL remplac� 0xFFFFFFFF;
         pPacket->dwAckCount   = dwMaxAck;
         if (dwMaxAck != 0) {
            pPacket->packetHeader->safe = 1; // This is a safe packet, needs an ack!
		 	if (pPacket->dwAckDelay<=0){//BLBLBL 13/12/2010, on teste voir si par hasard on ne donnerait pas un d�lai de 0 
...
void CCommCenter::ReSendPacket(UDPPacket* pPacket)
{
   ...
   pNewSendPack->packetHeader->safe     = pPacket->packetHeader->safe;    //ces donn�es sont crypt�es et n'ont aucun sens � �tre lues direct.
   ...
   pNewSendPack->dwAckDelay = ( pNewSendPack->dwAckDelay * 3 ) / 2;	
   	      ...
   	      if (pNewSendPack->dwAckDelay<=0){//BLBLBL 13/12/2010, on teste voir si par hasard on ne donnerait pas un d�lai de 0 
...
void CCommCenter::UDPSendDataThread(LPVOID lpData)
{
   ...
   while( lpComm->bUDPSendDataThread )
   {
	  ...
	  //bRet contient le r�sultat de l'envoi du packet
	  ...
	  //si l'envoi du packet a bien r�ussi :
	  if(bRet)
      {
         #ifndef USE_CLIENT_CONNECTION
            LEAVE_TIMEOUT
         ...
         {
            // Sets the time when the packet will need to be resent. //BLBLBL 04/12/2010 � voir ici
...
void CCommCenter::UDPAnalyseThread(LPVOID lpData)
{
   ...
   while( lpComm->bUDPAnalyseThread )
   {
      ...
      else bRet=false;//BLBLBL ajout d'un test pour �viter les cas de exception read/write to virtual adress blah blah
...
void CCommCenter::UDPSendPacketThread(LPVOID lpData)
{
   ...
   while( lpComm->bUDPSendPacketThread )
   {
      ...
      if(bRet)
      {
         ...
         //BLBL je vire �a c'est d�bile.
		 /*if(pPacket->packetHeader->Reserved)//BLBLBL Le Reserved est visiblement positionn� de temps en temps, au pif
		 ...
		 //if (pPacket->HeaderSave.safe )//BLBLBL Correctif de chaotik mind pour �tre sur de consulter une donn�e consultable..
         if (pPacket->needAck) //BLBLBL simplification du proc�d�
		 {
...
inline void CNMConnection::AddPending(UDPPacket* pPacket) 
{
	...
	if (m_pendingPackets.size() < MAX_PACKET_PENDING_LIST) 
   {
      ...
      pPacketPend->packetHeader->safe     = pPacket->packetHeader->safe;    //BLBLBL toutes ces valeurs sont crypt�es donc inutile de les acc�der
      ...
      //BLBLBL apparement pour le dwTimeout on peut/doit mettre 0xFFFFFFFF dans le cas d'un safe=0, � voir donc..
	  ...
	  //BLBLBL les packets au d�part diff�r� sont empil�s ici :
...
bool CNMConnection::DestroyPending (WORD packetID) 
{
   ...
   {
		if ((*i)->ID == packetID)
		//if ((*i)->packetHeader->packetID == packetID) //BLBLBL 04/12/2010 � voir ici, crash potentiel
		//if ((*i)->HeaderSave.packetID == packetID)//BLBLBL correctif de chaotik
      {
		 //BLBLBL a voir si on entre r�ellement dans ce bloc d'instructions
		 //il est possible que le header soit chiffr� et que du coup le packetID soit illisible ?
...
inline void CNMConnection::VerifyTimedoutPending()
{
   ...
   if (m_pendingPackets.size()) {//BLBLBL 04/12/2010 on teste si y a une liste d�j�..
   ...
   while ( i != m_pendingPackets.end() ) //BLBLBL 04/12/2010 probleme ici dans l'incr�mentation du pointeur de liste !! il semble que le pointeur i passe par des valeur qui ne sont pas dans la liste de l'it�rateur, � voir probleme de ++i ou i++ ???
   {

      if ( (*i) != NULL && ( (*i)->dwTimeout <= dwCurrentTime ) )//BLBLBL ajout� un test de pointeur null 
      {         
		 pResentPacket = *i;//03/12/2010 BLBLBL �a crash ici ? on assigne un pointeur d'it�rateur � un packet ?
         if(pResentPacket!=NULL)//BLBLBL ajout� d'un test pointeur null
         {

            // It timed out, lets get it ready for retransmission!            	
			pResentPacket->dwTimeout = GetRunTime() + pResentPacket->dwAckDelay;//BLBLBL remplac� 0xFFFFFFFF;//BLBLBL *i => pResentPacket
            ...
            if(pResentPacket->dwAckCount == 0)
            {
			   ...
			   //++i;//on fait pointer l'iterator sur le pak suivant de la liste//erase renvoie d�j� le pointeur suivant de l'iterateur
            ...
            else
			{
				...
				m_CommCenter->ReSendPacket(pResentPacket);//BLBLBL 06/12/2010 ajout� des braces pour faire la suppression OU l'envoi.
				++i;//on fait pointer l'iterator sur le pak suivant de la liste
			}

			//Send the packet to Sending queue
            //m_CommCenter->ReSendPacket(pResentPacket);//BLBLBL on peut pas envoyer si il a �t� supprim�.. d�plac� dans les braces ^^
         }
         else
            ++i;//on fait pointer l'iterator sur le pak suivant de la liste
