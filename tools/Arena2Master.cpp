...
int              Arena2Master::m_iNbrArena;
sArenaFlagGame  *Arena2Master::m_pArenaList = NULL;
...
// Creates the class
//////////////////////////////////////////////////////////////////////////////////////////
void Arena2Master::Create(int iNbrArena)
{
   ...
   for(int i=0;i<m_iNbrArena;i++)
   {
      ...
      m_pArenaList[i].c_itArene = theApp.CombatArenaLocationList2.begin();
...
// 
//////////////////////////////////////////////////////////////////////////////////////////
void Arena2Master::Destroy( void )
{
   if(m_iNbrArena <=0)
      return;
      ...
      else if(m_pArenaList[a].c_AreneStatus == 2)
      {
         ...
         for(int i=0;i<m_pArenaList[a].c_aTakeFlagList1.GetSize();i++)
         {
            ...
            if(lpIT->uiDecompte == 0)
            {
               ...
               if(lpIT->pPlayer->self->GetArenaTeam() == 1) //drapeau de SON EQUIPE
               {
                  // on repod le drapeau dans sa zone...
                  SummonFlagHome(0,a);
                  strTmp.Format(_STR( 15498, lpIT->pPlayer->self->GetLang() ),lpIT->pPlayer->self->GetTrueName());
                  SendMessageToAll(strTmp,CL_BLUE_2,a);
               }
               else
               {
                  //Drapeau equipe adverse on le prend...
                  lpIT->pPlayer->self->SetFlag(__FLAG_PLAYER_ARENE_HAVE_FLAG,1); //a drapeau BLEUE
                  lpIT->pPlayer->self->SetFlag(__FLAG_NMS_TAG_DISPLAY_OVER_HEAD,5); //flag bleue dessus sa tete
                  ...
                  strTmp.Format(_STR( 15500, lpIT->pPlayer->self->GetLang() ),lpIT->pPlayer->self->GetTrueName());
                  SendMessageToAll(strTmp,CL_RED,a);
                  m_pArenaList[a].c_FlagBInHome = 0;
         ...
         bFlagTaked = FALSE;
         //Process le FLAG 2
         for(int i=0;i<m_pArenaList[a].c_aTakeFlagList2.GetSize();i++)
         {
            lpIT = (sArenaTakeItemList *)m_pArenaList[a].c_aTakeFlagList2.GetAt(i);
            lpIT->uiDecompte--;
            if(lpIT->uiDecompte == 0)
            {
               CString strTmp;
               ...
               //on vaporize cet items et set le flag a ce user pour dire quil a pris le flag...
               lpIT->pItemUnit->VaporizeUnit(false);
               if(lpIT->pPlayer->self->GetArenaTeam() == 2) //drapeau de SON EQUIPE
               {
                  // on repod le drapeau dans sa zone...
                  SummonFlagHome(1,a);
                  strTmp.Format(_STR( 15499, lpIT->pPlayer->self->GetLang() ),lpIT->pPlayer->self->GetTrueName());
                  SendMessageToAll(strTmp,CL_RED,a);
               }
               else
               {
                  //Drapeau equipe adverse on le prend...
                  lpIT->pPlayer->self->SetFlag(__FLAG_PLAYER_ARENE_HAVE_FLAG,2); //a drapeau BLEUE
                  lpIT->pPlayer->self->SetFlag(__FLAG_NMS_TAG_DISPLAY_OVER_HEAD,6); //flag bleue dessus sa tete

                  TFCPacket sending;
                  lpIT->pPlayer->self->PacketPuppetInfo( sending );
                  Broadcast::BCast( lpIT->pPlayer->self->GetWL(), _DEFAULT_RANGE, sending, lpIT->pPlayer->self->GetInvisibleQuery() );

                  strTmp.Format(_STR( 15501, lpIT->pPlayer->self->GetLang() ),lpIT->pPlayer->self->GetTrueName());
                  SendMessageToAll(strTmp,CL_BLUE_2,a);
                  m_pArenaList[a].c_FlagRInHome = 0;
               }
               ...
               delete lpIT;
               lpIT = NULL;
               m_pArenaList[a].c_aTakeFlagList2.RemoveAt(i);
               i = m_pArenaList[a].c_aTakeFlagList2.GetSize(); //sort de la boucle...
            }
            else
            {
               SendFlagTakeStatus(lpIT->pPlayer,0,lpIT->uiDecompte,a);
         ...
         {
            //remove all flag waiting
            while(m_pArenaList[a].c_aTakeFlagList2.GetSize())
            {
               lpIT = (sArenaTakeItemList *)m_pArenaList[a].c_aTakeFlagList2.GetAt(0);
               SendFlagTakeStatus(lpIT->pPlayer,1,m_pArenaList[a].c_ArenaCfg.iNbrPlayTakeItemSec,a,true);
               delete lpIT;
               lpIT = NULL;
               m_pArenaList[a].c_aTakeFlagList2.RemoveAt(0);
            }
         }
         ...
         else
         {
            ...
            if(uiEndGame == ARENA1_END_TIMEOUT)
            {
               _LOG_ARENA
                  LOG_ALWAYS,
                  "ARENE2 ID %d: TIMEOUT blue/red %d/%d",a,
            ...
            else if(uiEndGame == ARENA1_END_COMPLETED)
            {
               _LOG_ARENA
                  LOG_ALWAYS,
                  "ARENE2 ID %d: ENDED blue/red %d/%d",a,
                  m_pArenaList[a].c_PointBTeam,m_pArenaList[a].c_PointRTeam
                  LOG_
            }
            else if(uiEndGame == ARENA1_END_NO_PLAYER)
            {
               _LOG_ARENA
                  LOG_ALWAYS,
                  "ARENE2 ID %d: NO Player left on a team",a
                  LOG_
            }
            else if(uiEndGame == ARENA1_END_BALANCE_PLAYER)
            {
               _LOG_ARENA
                  LOG_ALWAYS,
                  "ARENE2 ID %d: Number of team player > 2",a
                  LOG_
            }
...
void Arena2Master::IncreasePoint(Players *pPlayer, int iAreneID)
{
...
void Arena2Master::AddPlayer(Players *pPlayer, int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
   ...
   if(m_pArenaList[iAreneID].c_AreneStatus == 0)
   {
      ...
      pPlayer->self->SetArenaType(ARENE2_TYPE);
      ...
      _LOG_ARENA
         LOG_ALWAYS,
         "ARENE2 ID %d: Player ID:%d %s (%s) CREATE Arene level range %d - %d",iAreneID,
         pPlayer->self->GetID(),
   ...
   else if(m_pArenaList[iAreneID].c_AreneStatus == 1)
   {
      ...
      _LOG_ARENA
         LOG_ALWAYS,
         "ARENE2 ID %d: Player ID:%d %s (%s) JOIN ARENA (waiting) level range %d - %d",iAreneID,
         pPlayer->self->GetID(),
         pPlayer->self->GetTrueName(),
         pPlayer->GetFullAccountName(),
         m_pArenaList[iAreneID].c_iLevelMin,m_pArenaList[iAreneID].c_iLevelMax
...
void Arena2Master::RemovePlayer(Players *pPlayer,bool bUpdateList, int iAreneID,BOOL bPenalty)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
   ...
   if(m_pArenaList[iAreneID].c_AreneStatus == 1)
   {
      ...
      _LOG_ARENA
         LOG_ALWAYS,
         "ARENE2 ID %d: Player ID:%d %s (%s) LEAVE Arene in waiting mode",iAreneID,
         pPlayer->self->GetID(),
         pPlayer->self->GetTrueName(),
         pPlayer->GetFullAccountName()
         LOG_
      ...
      if(m_pArenaList[iAreneID].c_aPlayerList.GetSize() <=0)
      {
         _LOG_ARENA
            LOG_ALWAYS,
            "ARENE2 ID %d: END of Arene, No player left on waiting mode",iAreneID
            LOG_
   ...
   else if(m_pArenaList[iAreneID].c_AreneStatus == 2)
   {
      ...
      for(UINT i=0;i<m_pArenaList[iAreneID].c_aPlayerList.GetSize();i++)
      {
         ...
         if(pPlayer->self->GetID() == lpPL->pPlayer->self->GetID())
         {
            int iHaveFlag = pPlayer->self->ViewFlag(__FLAG_PLAYER_ARENE_HAVE_FLAG);
            if(iHaveFlag >0)
            {
               //on dois remettre le drapeau en jeu...
               SummonFlag(iHaveFlag-1,iAreneID,pPlayer->self->GetWL().X,pPlayer->self->GetWL().Y,pPlayer->self->GetWL().world);
            }
      ...
      _LOG_ARENA
         LOG_ALWAYS,
         "ARENE2 ID %d: Player ID:%d %s (%s) LEAVE Arene in playing mode",iAreneID,
         pPlayer->self->GetID(),
         pPlayer->self->GetTrueName(),
         pPlayer->GetFullAccountName()
         LOG_
...
void Arena2Master::SendArenaList(Players *pPlayer, int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
...
void Arena2Master::SendArenaPlayStat(Players *pPlayer,bool bForceOffline, int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
...
void Arena2Master::SendArenaWaitingList(Players *pPlayer, int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
...
void Arena2Master::SendPlayerFakeGroup(Players *pPlayer, int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
...
void Arena2Master::ResetPlayerArene(Players *pPlayer, int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;

   RemTakeList(0,pPlayer,iAreneID);
   RemTakeList(1,pPlayer,iAreneID);
...
void Arena2Master::SummonFlagHome(int iID,int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
   ...
   if(iID == 0)
   {
      SummonFlag(0,iAreneID,(*m_pArenaList[iAreneID].c_itArene).wlItemPod1.X,(*m_pArenaList[iAreneID].c_itArene).wlItemPod1.Y,(*m_pArenaList[iAreneID].c_itArene).wlItemPod1.world);
      m_pArenaList[iAreneID].c_FlagBInHome = 1;
   }
   else if(iID == 1)
   {
      SummonFlag(1,iAreneID,(*m_pArenaList[iAreneID].c_itArene).wlItemPod2.X,(*m_pArenaList[iAreneID].c_itArene).wlItemPod2.Y,(*m_pArenaList[iAreneID].c_itArene).wlItemPod2.world);
      m_pArenaList[iAreneID].c_FlagRInHome = 1;
   }
}

void Arena2Master::SummonFlag(int iID,int iAreneID,int iX, int iY, int iW)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
   ...
   DWORD dwID = 0;
   if(iID == 0)
      dwID = (*m_pArenaList[iAreneID].c_itArene).iItemID1;
   else if(iID == 1)
      dwID = (*m_pArenaList[iAreneID].c_itArene).iItemID2;
   ...
   {  
      ...
      else
      {
         _LOG_ARENA
            LOG_ALWAYS,
            "ARENE2 ID %d: Unable to summon flag... invalid position",iAreneID
            LOG_
         lpItem->DeleteUnit();
      }
   ...
   else
   {
      _LOG_ARENA
         LOG_ALWAYS,
         "ARENE2 ID %d: Unable to summon flag... invalid item",iAreneID+1
         LOG_
   }
}
...
void Arena2Master::SendMessageToAll(CString strMessage,DWORD dwColor, int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
...
void Arena2Master::SendMessageToTeam(CString strPlayerName,int iTeamID,CString strMessage, int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
...
void Arena2Master::AddTakeList(int iItemID,Players *pPlayer,Unit *pItemUnit, int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
   ...
   {
      int iPos = m_pArenaList[iAreneID].c_aTakeFlagList2.GetSize();
      if(iPos <0) iPos = 0;
      m_pArenaList[iAreneID].c_aTakeFlagList2.SetAtGrow(iPos, pNewItem );
   }
...
void Arena2Master::RemTakeList(int iItemID,Players *pPlayer, int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
   ...
   if(iItemID == 0)
   {
      for(int i=0;i<m_pArenaList[iAreneID].c_aTakeFlagList1.GetSize();i++)
      {
         ...
         {
            SendFlagTakeStatus(pItemL->pPlayer,1,m_pArenaList[iAreneID].c_ArenaCfg.iNbrPlayTakeItemSec,iAreneID,true);
            delete pItemL;
            pItemL = NULL;
   ...
   {
      for(int i=0;i<m_pArenaList[iAreneID].c_aTakeFlagList2.GetSize();i++)
      {
         pItemL = (sArenaTakeItemList *)m_pArenaList[iAreneID].c_aTakeFlagList2.GetAt(i);
         if(pItemL->pPlayer->self->GetID() == pPlayer->self->GetID())
         {
            SendFlagTakeStatus(pItemL->pPlayer,1,m_pArenaList[iAreneID].c_ArenaCfg.iNbrPlayTakeItemSec,iAreneID,true);
            delete pItemL;
            pItemL = NULL;
            m_pArenaList[iAreneID].c_aTakeFlagList2.RemoveAt(i);
            i = m_pArenaList[iAreneID].c_aTakeFlagList2.GetSize(); //sort de la boucle...
         }
      }
   }
...
void Arena2Master::SendFlagTakeStatus(Players *pPlayer, char chCode, char chDecompte, int iAreneID,bool bForceZero)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return;
...
int Arena2Master::GetNumberOfArene()
{
   return m_iNbrArena;
}

CString Arena2Master::GetAreneName(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return "";
...
int Arena2Master::GetSummonItemID1(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;
...
int Arena2Master::GetSummonItemID2(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;

   return (*m_pArenaList[iAreneID].c_itArene).iItemID2;
}


int  Arena2Master::GetItemID1Home(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;

   return m_pArenaList[iAreneID].c_FlagBInHome;
}

int  Arena2Master::GetItemID2Home(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;

   return m_pArenaList[iAreneID].c_FlagRInHome;
}


WorldPos Arena2Master::GetRecallDeathTeam(int iTeamID,int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
   {
...
int  Arena2Master::GetDeathWaitTimeMS(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;
...
int  Arena2Master::GetDeathWaitTimeS(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;
...
int  Arena2Master::GetMaxMinutes(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;
...
int  Arena2Master::GetMaxPoints(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;
...
int  Arena2Master::GetArenaStatus(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;
...
int  Arena2Master::GetArenaMinLevel(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;
...
int  Arena2Master::GetArenaMaxLevel(int iAreneID)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;
...
int  Arena2Master::GetArenaWaitTimeGen(int iAreneID,int &iNbrSec,int &iNbrMin)
{
   if(iAreneID <0 || iAreneID >= m_iNbrArena)
      return 0;
