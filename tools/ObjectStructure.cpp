...
Add Spell activation on the OnEquip()
Add Spell Desactivation on the OnUnequip() by Nightmare (27/06/2009)
/******************************************************************************/
#include "stdafx.h"
...
void ObjectStructure::OnInitialise( UNIT_FUNC_PROTOTYPE )
/******************************************************************************/
{
...
void ObjectStructure::OnAttack( UNIT_FUNC_PROTOTYPE )
 // medium	attacker.
 // target	attacked.
 // self	this item.
/******************************************************************************/
...
/******************************************************************************/
// When attecked.
void ObjectStructure::OnAttacked( UNIT_FUNC_PROTOTYPE )
 // medium	victim.
 // target	attacker.
 // self	this item.
/******************************************************************************/
...
void ObjectStructure::OnHit( UNIT_FUNC_PROTOTYPE )
 // medium	victim.
 // target	attacker.
 // self	this item.
/******************************************************************************/
...
void ObjectStructure::OnAttackHit( UNIT_FUNC_PROTOTYPE )
 // medium	attacker.
 // target	attacked.
 // self	this item.
/******************************************************************************/
...
void ObjectStructure::OnDisturbed( UNIT_FUNC_PROTOTYPE )
 // self		this item.
...
void ObjectStructure::OnNoMoreShots( UNIT_FUNC_PROTOTYPE)
...
/******************************************************************************/
// When used.
void ObjectStructure::OnUse( UNIT_FUNC_PROTOTYPE )
...
void ObjectStructure::OnEquip( UNIT_FUNC_PROTOTYPE )
...
/******************************************************************************/
{
	...
	while( tlSpells.QueryNext() ){
      ...
      {		            
			if( nCharges != 0 )
         {
            ...
            {
               if(self->GetStaticReference() == theApp.m_aDelayItems[i].uiID)
               {
                  time_t tCurTime  =  time(NULL);
                  time_t tLastTime =  target->ViewFlag(theApp.m_aDelayItems[i].uiFlag);
                  if(tCurTime - tLastTime > theApp.m_aDelayItems[i].uiDelay)
                  {
                     target->SetFlag(theApp.m_aDelayItems[i].uiFlag,tCurTime);
                     i = theApp.m_aDelayItems.GetSize();
                  }
                  else
                  {
                     CString strMessage;
                     strMessage.Format(_STR( 15371, target->GetLang() ));
                     target->SendSystemMessage(strMessage);
                     bCanExecute = false;
                     i = theApp.m_aDelayItems.GetSize();
                  }
               }
   ...
   // First trigger all timer effects.
   self->VerifyTimers();
   self->QueryEffects( MSG_OnEquip, NULL, self, target );
}
/******************************************************************************/
// When the object is being unequipped.
void ObjectStructure::OnUnequip( UNIT_FUNC_PROTOTYPE )
...
/******************************************************************************/
{
	...
	while( tlSpells.QueryNext() ){
		...
		// If there is a spell to trigger.
		if( lpSpell->wHook == MSG_OnUnequip ){		            
			if( nCharges != 0 ){
				WorldPos wlPos = target == NULL ? self->GetWL() : target->GetWL();			
				SpellMessageHandler::ActivateSpell( lpSpell->wSpellID, target, self, target, wlPos );
                nCharges--;
    ...
    // First trigger all timer effects.
    self->VerifyTimers();
    self->QueryEffects( MSG_OnUnequip, NULL, self, target );
}