...
ENDIF
IF (DWORD(CheckNPCFlag( __NPC_FLAG_GEENA_INFECTED)) > CurrentRound)
   IF (CheckItem(__OBJ_HEALING_LEAF) > 0)
	...
	ELSE
      ...
      BREAK
   ENDIF
ELSEIF ((DWORD(CheckNPCFlag( __NPC_FLAG_GEENA_INFECTED )) < CurrentRound) && (CheckNPCFlag( __NPC_FLAG_MURMUNTAG_INFECTED ) != 0 ))
	Conversation
