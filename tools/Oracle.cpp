...
""
IF((CheckFlag(35000)  <= 0) && (CheckFlag(__FLAG_NUMBER_OF_REMORTS) > 99)) //le 99 fait en sorte que les renaissnace soir gratuites...
   Conversation
      FORMAT("Les Dieux n'ont pas encore reçu leurs offrandes...  Faites, et revenez me voir.")
ELSEIF(CheckFlag(__FLAG_USER_HAS_DEFEATED_ASSISTANT) == 1)

	IF(DWORD(CheckFlag(__FLAG_NUMBER_OF_REMORTS)) >= ACK_MAXREMORTS)		
		Conversation
