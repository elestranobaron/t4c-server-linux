...
/******************************************************************************/
{
    ...
    npc.InitialPos.X = 2844;//1415; // Somewhere in Silversky castle
    npc.InitialPos.Y = 691;//2334; // Somewhere in Silversky castle
...
/******************************************************************************/
{
	InitTalk
		...
		Begin
			...
			IF (DWORD(CheckGlobalFlag(__GLOBAL_QUEST_CROWBANNER_TIMER)) <= CurrentRound)
		...
		Command3(INTL( 700, "SPELL"),INTL( 495, "LEARN"),INTL( 496, "TEACH"))
			...
			CreateSkillList
				AddTeachSkill (__SPELL_SANCTUARY, 33, 99632)
	...
	NPC_DATA_EXCHANGE
		NPC_DATA_TEACH
			NPC_TEACH_SKILL_LIST
				TEACH_SKILL(__SPELL_SANCTUARY, 33, 99632)
