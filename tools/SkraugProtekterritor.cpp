...
extern NPCstructure::NPC SkraugProtekterritorNPC;

void SkraugProtekterritor::Create(){
   npc = SkraugProtekterritorNPC;
   SET_NPC_NAME( "[10925]Skraug protekterritor" );
   npc.InitialPos.X = 0; 
   npc.InitialPos.Y = 0; 
	npc.InitialPos.world = 0;
}

void SkraugProtekterritor::OnAttack( UNIT_FUNC_PROTOTYPE )
{  
	switch(rnd(0, 40))	 
	{		
...
void SkraugProtekterritor::OnAttacked( UNIT_FUNC_PROTOTYPE )
{ 
	switch(rnd(0, 40))	 
	{
...
void SkraugProtekterritor::OnDeath( UNIT_FUNC_PROTOTYPE )
{
	switch(rnd(0, 40))	 
	{	
