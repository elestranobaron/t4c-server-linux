...
//									obj.name =_strdup( newname );
#define SET_NAME( obj, newname )	if( obj.name ) delete obj.name;\
                                 obj.name = new char[ strlen( newname ) + 1];\
                                 strcpy_s( obj.name,strlen( newname ) + 1, newname );
...
struct _item
{
   ...
   DWORD dwBuySellFlagID;
   DWORD costFixed;				// Price of item when sold.
   BoostFormula costFormula;  //formula for the item price...
