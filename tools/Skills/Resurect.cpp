...
{    
   ...
   else
   {
      ...
      //Desactiver le combat quand le skill est utilisé
      if( self->GetType() == U_PC )
      {
         Character *ch = static_cast< Character * >( self );
         ch->StopAutoCombat();
      ...
      if( nRange <= ResurectRange  ) 
      {
         ...
         // La competence n'est pas passée
         else 
         {
            self->SendInfoMessage( _STR( 15017, self->GetLang() ),0x000080 );
         }
      ...
      lostGold = self->ViewFlag(__FLAG_DEATH_LOST_GOLD, 0); // On récupère l'or perdu par la cible
      if(lostGold > 0)
      {
         // On récupère un pourcentage d'or, si on a ressucité à 100% on récupère 50% de l'or. (on peut monter à un bonus de 200% pour récuperer tout l'or)
         if (dLvlRatio < 200) 
         {
