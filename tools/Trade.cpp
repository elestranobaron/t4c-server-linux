...
/******************************************************************************/
TradeMgr2::TradeMgr2(Character &ownerCharacter) : 
	m_ThisCharacter(ownerCharacter), mOtherTrade(NULL), m_Status(Status::TS_Invalid),
	m_StatusCharacter(Status::EditingItems), m_Container(new ItemContainer())
/******************************************************************************/
{
}
/******************************************************************************/
...
{
	// CheckInitialization();
	if (&pInvitedTradeMgr == this)
	{
		return ErrorCodes::InvalidParameters;
	}
	...
	{ // autolock scoping..
		...
		// Are they in range?
		BOOL charactersAreCloseEnough = m_ThisCharacter.GetWL().AreInRange(invitedCharacter.GetWL(), 10);
		...
		else 
		{
			...
			if (InvitorIsInvitingSomeoneElse)
			{
				// Cancel any other invite made before to others
				Cancel();
			}
...
/******************************************************************************/
{
	...
	if (m_Status == Status::TS_Inviting) 
	{
		// Cancel an invite..
		ResetTradeData();
		return ErrorCodes::InviteCancelled;
	}
	else if (m_Status == Status::TS_Trading) 
	{
		// Cancel a running trade..
		//DEVOLVER OS ITEMS!
...
/******************************************************************************/
{
	...
	if (m_Status == Status::TS_Invalid) 
	{
		return ErrorCodes::NoTradeToFinish;
	}
	if (m_Status == Status::TS_Inviting) 
	{
		// Cant finish an invite
		return ErrorCodes::CantFinishAnInvite;
	} 
	else if (m_Status == Status::TS_Trading) 
	{
		// Its a running trade..
		if (m_StatusCharacter == mOtherTrade->m_StatusCharacter && m_StatusCharacter == TradeMgr2::Status::Confirmed) 
		{
			// Both chars status is "Confirmed"
			//Do item switching!
...
/******************************************************************************/
{
    _LOG_ITEMS
        LOG_MISC_1,
		"Finished a trade between characters [ %s (%s) ] and [ %s (%s) ]",
		m_ThisCharacter.GetName(),
      m_ThisCharacter.GetPlayer()->GetFullAccountName(),
		mOtherTrade->m_ThisCharacter.GetName(),
      mOtherTrade->m_ThisCharacter.GetPlayer()->GetFullAccountName()
    LOG_
    _LOG_ITEMS
        LOG_MISC_1,
		"\tCharacter %s (%s) received: %s",
		m_ThisCharacter.GetName(),
      m_ThisCharacter.GetPlayer()->GetFullAccountName(),
		pMyItems
    LOG_
...
/******************************************************************************/
{
	...
	if (m_Status != Status::TS_Trading)
	{
		return ErrorCodes::TradeInvalidForItemRemoving;
	}
...
TradeMgr2::SetCharacterStatus(TradeMgr2::Status::CharacterStatus newStatus) 
{
	...
	if (m_Status == Status::TS_Invalid) 
	{
		return TradeMgr2::ErrorCodes::NoTradeToSetCharacterStatus;
	}
...
/******************************************************************************/
{
	...
	//m_ThisCharacter.SendBackpackContentPacket(); // send an updated backpack Plus besoin car s apeu causer deadlock...
