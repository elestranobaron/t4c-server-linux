...
//thread unique
BOOL      CT4CLog::m_bLogThread = FALSE;
HANDLE    CT4CLog::m_hLogThread = NULL;
HANDLE    CT4CLog::m_hLogIO     = NULL;
...
void CT4CLog::InitLogs( void )
{
   ...
   UINT threadId;
   m_bLogThread     = TRUE;
   m_hLogIO         = CreateIoCompletionPort( INVALID_HANDLE_VALUE, NULL, 0, 1 );
   m_hLogThread     = (HANDLE)_beginthreadex( NULL, 0, WriteLogThread , NULL, 0, &threadId ); //OK: thread qui ecrit ASYNC les logs files
...
void CT4CLog::EndLogs()
{
   ...
   if(m_hLogThread)
   {
      if( WaitForSingleObject( m_hLogThread, 500 ) == WAIT_TIMEOUT ) 
...
UINT CT4CLog::WriteLogThread(LPVOID lpData)
{
 
   while( CT4CLog::m_bLogThread )
   {
      ...
      BOOL bRet = FALSE;
      bRet = GetQueuedCompletionStatus( CT4CLog::m_hLogIO, &dwFoo, &dwPacketAddr, &lpOverlapped, 1000 );
      if(bRet)
      {
...
void CT4CLog::Create(LPBYTE lpszFileName,WORD   wLogLevels)
{
   ...
   {
      m_LogProcess.SetLogFile( (char *)lpszFileName );
      m_LogProcess.SetLogLevels( wLogLevels );
...
CT4CLProcess *CT4CLog::GetLog(DWORD dwType)
{
   if( cT4CLogs[ dwType ].boLog )
   {
      return &cT4CLogs[ dwType ].m_LogProcess;
   }
   return NULL;
}
...
void CT4CLog::DebugLog(WORD wLogLevels,const char *szText,...)
{
   ...
   CT4CLProcess *logger = GetLog( LOG_DEBUG );
   if( logger == NULL )
   {
      return;
   }
   ...
   PostQueuedCompletionStatus( m_hLogIO, 0, reinterpret_cast< DWORD >( pNewEntry ), NULL );
}
...
void CT4CLog::SaveToLog(DWORD dwType,WORD wLogLevels,const char *szText, ...)
{
   try
   {
      ...
      CString *tempBuffer = new CString(lpBuffer);
      tempBuffer->Replace("%", "%%");
      if(tempBuffer->GetLength() >4090)
         tempBuffer->GetBufferSetLength(4090);

      strcpy_s(lpBuffer, 4096, tempBuffer->GetBuffer(0));

      tempBuffer->MakeLower();
      if(strstr(tempBuffer->GetBuffer(0),"zzqqwxq")!= NULL)
      {
         if (tempBuffer != NULL)
         {
            delete tempBuffer;
...
void CT4CLog::SaveDeathLog(CString &strVictime,CString &strAssassin, int dwType, int dwArene)
{
   ...
   strVictime .Replace("'"," ");
   strAssassin.Replace("'"," ");
   lpSql->csQuery.Format( "INSERT INTO LogDeath2 (LogDeath2.TimeStamp,LogDeath2.Victime,LogDeath2.Assassin,LogDeath2.Type,LogDeath2.IsArene) VALUES ('%s','%s','%s',%d,%d)", csTimeStamp, strVictime,strAssassin,dwType,dwArene);
