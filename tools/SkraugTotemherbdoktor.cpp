...
extern NPCstructure::NPC SkraugTotemherbdoktorNPC;

void SkraugTotemherbdoktor::Create(){
   npc = SkraugTotemherbdoktorNPC;
   SET_NPC_NAME( "[10927]Skraug totemherbdoktor" );
   npc.InitialPos.X = 0; 
   npc.InitialPos.Y = 0; 
   npc.InitialPos.world = 0;
}

void SkraugTotemherbdoktor::OnAttack( UNIT_FUNC_PROTOTYPE )
{  
	switch(rnd(0, 40))	 
	{		
...
void SkraugTotemherbdoktor::OnAttacked( UNIT_FUNC_PROTOTYPE )
{ 
	switch(rnd(0, 40))	 
	{
...
void SkraugTotemherbdoktor::OnDeath( UNIT_FUNC_PROTOTYPE )
{
	switch(rnd(0, 40))	 
	{	
