...
extern NPCstructure::NPC SkraugMeanheaddrummahNPC;

void SkraugMeanheaddrummah::Create(){
   npc = SkraugMeanheaddrummahNPC;
   SET_NPC_NAME( "[10922]Skraug meanheaddrummah" );
   npc.InitialPos.X = 0; 
   npc.InitialPos.Y = 0; 
	npc.InitialPos.world = 0;
}

void SkraugMeanheaddrummah::OnAttack( UNIT_FUNC_PROTOTYPE )
{  
	switch(rnd(0, 40))	 
	{		
...
void SkraugMeanheaddrummah::OnAttacked( UNIT_FUNC_PROTOTYPE )
{ 
	switch(rnd(0, 40))	 
	{
...
void SkraugMeanheaddrummah::OnDeath( UNIT_FUNC_PROTOTYPE )
{
	switch(rnd(0, 40))	 
	{	
