...
Add New type x: Ajout de plein de type de stats... by Nightmare :Totest (60/06/2009)
/******************************************************************************/
#include "stdafx.h"
...
#define	TYPE_SELF_ID	146
#define	TYPE_TARGET_ID	147
#define	TYPE_SELF_STATPOINTS	148
#define	TYPE_TARGET_STATPOINTS	149
#define	TYPE_SELF_SKILLPOINTS	150
#define	TYPE_TARGET_SKILLPOINTS	151
#define	TYPE_SELF_TRUE_AC	152
#define	TYPE_TARGET_TRUE_AC	153
#define	TYPE_SELF_LUCK	154
#define	TYPE_TARGET_LUCK	155
#define	TYPE_SELF_TRUE_LUCK	156
#define	TYPE_TARGET_TRUE_LUCK	157
#define	TYPE_TARGET_STATICID    158
...
#define FUNC_SELF_VIEW_FRIENDLY     17
#define FUNC_TARGET_VIEW_FRIENDLY   18
...
BOOL BoostFormula::SetFormula(CString csFormula) // The formula
/******************************************************************************/
{
    BOOL boReturn = TRUE;
    ...
    if( !csFormula.IsEmpty() )
	{
        ...
        //TRACE( "\r\n--Formula before parsing: \"%s\"", lpszFormula ); //NMNMNM ben t5rop ed trace...
...
/******************************************************************************/
{
	...
	else if( lpOperand->bFunction != 0 )
	{
        switch( lpOperand->bFunction )
		{
            ...
            case FUNC_SELF_VIEW_FRIENDLY:
            {
               dblFinalValue = 0;
               DWORD dwMonsterFID = GetOperandValue( lpOperand->lpLeftOperand, self, target, dblX, dblY, dblR, varMap, method );
               DWORD dwFactionID  = self->ViewFlag( __FLAG_PJ_VS_MONSTER_FRIENDLY );
            ...
            break;
            case FUNC_TARGET_VIEW_FRIENDLY:
            {
               if( target != NULL )
               {
                  dblFinalValue = 0;
                  DWORD dwMonsterFID = GetOperandValue( lpOperand->lpLeftOperand, self, target, dblX, dblY, dblR, varMap, method );
                  DWORD dwFactionID = target->ViewFlag( __FLAG_PJ_VS_MONSTER_FRIENDLY );
                  if(dwFactionID >0 && dwFactionID <31 && dwMonsterFID > 0)
                  {
                     DWORD dwFlagID      = 3000000+dwMonsterFID;
                     DWORD dwFactionMask = PLFactionMask(dwFactionID);
	...
	else
	{
		switch( lpOperand->bValueType )
		{
			...
			case TYPE_INFINITE:             dblFinalValue = 1 * pow( 10.0, 308.0 ); break;
			...
			case TYPE_SELF_TRUE_AC:			dblFinalValue = self != NULL ? (self->GetType() == U_PC ? static_cast< Character * >(self)->GetTrueAC() : self->GetAC() ) : 0; break;
			case TYPE_TARGET_TRUE_AC:		dblFinalValue = target != NULL ? (target->GetType() == U_PC ? static_cast< Character * >(target)->GetTrueAC() : target->GetAC() ) : 0; break;
			case TYPE_TARGET_CRIME:			dblFinalValue = target != NULL ? target->GetCrime() : 0; break;
			case TYPE_SELF_CRIME:		    dblFinalValue = self != NULL ? self->GetCrime() : 0; break;
			case TYPE_TARGET_HONOR:			dblFinalValue = target != NULL ? target->GetHonor() : 0; break;
			case TYPE_SELF_HONOR:			dblFinalValue = self != NULL ? self->GetHonor() : 0; break;
			case TYPE_SELF_ID:				dblFinalValue = self != NULL ? (self->GetType() == U_PC ? self->GetID() : 0): 0; break;
			case TYPE_TARGET_ID:			dblFinalValue = target != NULL ? (target->GetType() == U_PC ? target->GetID() : 0): 0; break;
            case TYPE_TARGET_STATICID:		dblFinalValue = target != NULL ? target->GetStaticReference() : 0; break;
			case TYPE_SELF_STATPOINTS:		dblFinalValue = self != NULL ? ( self->GetType() == U_PC ? static_cast< Character * >(self)->GetStatPoints() : 0 ): 0; break;
			case TYPE_TARGET_STATPOINTS:	dblFinalValue = target != NULL ? ( target->GetType() == U_PC ? static_cast< Character * >(target)->GetStatPoints() : 0 ): 0; break;
			case TYPE_SELF_SKILLPOINTS:	    dblFinalValue = self != NULL ? self->GetSkillPoints() : 0; break;
			case TYPE_TARGET_SKILLPOINTS:	dblFinalValue = target != NULL ? target->GetSkillPoints() : 0; break;
...
{   
	...
	//TRACE( "\r\nParsing string %s.", lpszFormula ); //NMNMNM ben trop de log avec sa...
		...
		if( lpszFormula[ 0 ] == 0 )
		{
			...
			if( isdigit( lpszFormula[ 0 ] ) )
			{
				...
				if( lpszFormula[ i ] == '.' )
				{
					...
					while( isdigit( lpszFormula[ i ] ) )
					{
						lpOperand->dblValue += ( ( lpszFormula[ i ] - '0' ) * pow( 10.0, double(-nTenth) ) );
						i++;
						nTenth++;
					}
				}
				...
				if( isalpha( lpszFormula[ 0 ] ) )
				{
					...
					if( csFunc ==       "self.int" )						lpOperand->bValueType = TYPE_SELF_INT;                
					else if( csFunc == "self.str" )							lpOperand->bValueType = TYPE_SELF_STR;
					else if( csFunc == "self.end" )							lpOperand->bValueType = TYPE_SELF_END;
					else if( csFunc == "self.wis" )							lpOperand->bValueType = TYPE_SELF_WIS;
					else if( csFunc == "self.agi" )							lpOperand->bValueType = TYPE_SELF_AGI;                
					else if( csFunc == "self.hp" )							lpOperand->bValueType = TYPE_SELF_HP;
					else if( csFunc == "self.maxhp" )						lpOperand->bValueType = TYPE_SELF_MAXHP;
					else if( csFunc == "self.mana" )						lpOperand->bValueType = TYPE_SELF_MANA;
					else if( csFunc == "self.maxmana" )						lpOperand->bValueType = TYPE_SELF_MAXMANA;
					else if( csFunc == "self.attack" )						lpOperand->bValueType = TYPE_SELF_ATTACK;
					else if( csFunc == "self.dodge" )						lpOperand->bValueType = TYPE_SELF_DODGE;
					else if( csFunc == "self.level" )						lpOperand->bValueType = TYPE_SELF_LEVEL;
					else if( csFunc == "target.int" )						lpOperand->bValueType = TYPE_TARGET_INT;                
					else if( csFunc == "target.str" )						lpOperand->bValueType = TYPE_TARGET_STR;
					else if( csFunc == "target.end" )						lpOperand->bValueType = TYPE_TARGET_END;
					else if( csFunc == "target.wis" )						lpOperand->bValueType = TYPE_TARGET_WIS;
					else if( csFunc == "target.agi" )						lpOperand->bValueType = TYPE_TARGET_AGI;  
					else if( csFunc == "target.hp" )						lpOperand->bValueType = TYPE_TARGET_HP;
					else if( csFunc == "target.maxhp" )						lpOperand->bValueType = TYPE_TARGET_MAXHP;
					else if( csFunc == "target.mana" )						lpOperand->bValueType = TYPE_TARGET_MANA;
					else if( csFunc == "target.maxmana" )					lpOperand->bValueType = TYPE_TARGET_MAXMANA;
					else if( csFunc == "target.attack" )					lpOperand->bValueType = TYPE_TARGET_ATTACK;
					else if( csFunc == "target.dodge" )						lpOperand->bValueType = TYPE_TARGET_DODGE;
					else if( csFunc == "target.element" )					lpOperand->bValueType = TYPE_TARGET_ELEMENT;
					else if( csFunc == "target.level" )						lpOperand->bValueType = TYPE_TARGET_LEVEL;
					else if( csFunc == "x" || csFunc == "arrow_dmg" )		lpOperand->bValueType = TYPE_X_ARGUMENT;
					else if( csFunc == "y" )								lpOperand->bValueType = TYPE_Y_ARGUMENT;
					else if( csFunc == "r" )								lpOperand->bValueType = TYPE_R_ARGUMENT;
					else if( csFunc == "self.r_air" )						lpOperand->bValueType = TYPE_SELF_AIR_RESIST;
					else if( csFunc == "self.r_fire" )						lpOperand->bValueType = TYPE_SELF_FIRE_RESIST;
					else if( csFunc == "self.r_water" )						lpOperand->bValueType = TYPE_SELF_WATER_RESIST;
					else if( csFunc == "self.r_earth" )						lpOperand->bValueType = TYPE_SELF_EARTH_RESIST;
					else if( csFunc == "self.air" )							lpOperand->bValueType = TYPE_SELF_AIR_POWER;
					else if( csFunc == "self.fire" )						lpOperand->bValueType = TYPE_SELF_FIRE_POWER;
					else if( csFunc == "self.water" )						lpOperand->bValueType = TYPE_SELF_WATER_POWER;
					else if( csFunc == "self.earth" )						lpOperand->bValueType = TYPE_SELF_EARTH_POWER;
					else if( csFunc == "target.r_air" )						lpOperand->bValueType = TYPE_TARGET_AIR_RESIST;
					else if( csFunc == "target.r_fire" )					lpOperand->bValueType = TYPE_TARGET_FIRE_RESIST;
					else if( csFunc == "target.r_water" )					lpOperand->bValueType = TYPE_TARGET_WATER_RESIST;
					else if( csFunc == "target.r_earth" )					lpOperand->bValueType = TYPE_TARGET_EARTH_RESIST;
					else if( csFunc == "target.air" )						lpOperand->bValueType = TYPE_TARGET_AIR_POWER;
					else if( csFunc == "target.fire" )						lpOperand->bValueType = TYPE_TARGET_FIRE_POWER;
					else if( csFunc == "target.water" )						lpOperand->bValueType = TYPE_TARGET_WATER_POWER;
					else if( csFunc == "target.earth" )						lpOperand->bValueType = TYPE_TARGET_EARTH_POWER;
					else if( csFunc == "self.r_light" )						lpOperand->bValueType = TYPE_SELF_LIGHT_RESIST;
					else if( csFunc == "self.r_dark" )						lpOperand->bValueType = TYPE_SELF_DARK_RESIST;
					else if( csFunc == "self.light" )						lpOperand->bValueType = TYPE_SELF_LIGHT_POWER;
					else if( csFunc == "self.dark" )						lpOperand->bValueType = TYPE_SELF_DARK_POWER;
					else if( csFunc == "target.r_light" )					lpOperand->bValueType = TYPE_TARGET_LIGHT_RESIST;
					else if( csFunc == "target.r_dark" )					lpOperand->bValueType = TYPE_TARGET_DARK_RESIST;
					else if( csFunc == "target.light" )						lpOperand->bValueType = TYPE_TARGET_LIGHT_POWER;
					else if( csFunc == "target.dark" )						lpOperand->bValueType = TYPE_TARGET_DARK_POWER;
					else if( csFunc == "self.true_int" )					lpOperand->bValueType = TYPE_SELF_TRUE_INT;
					else if( csFunc == "self.true_str" )					lpOperand->bValueType = TYPE_SELF_TRUE_STR;
					else if( csFunc == "self.true_end" )					lpOperand->bValueType = TYPE_SELF_TRUE_END;
					else if( csFunc == "self.true_wis" )					lpOperand->bValueType = TYPE_SELF_TRUE_WIS;
					else if( csFunc == "self.true_agi" )					lpOperand->bValueType = TYPE_SELF_TRUE_AGI;
					else if( csFunc == "self.true_attack" )					lpOperand->bValueType = TYPE_SELF_TRUE_ATTACK;
					else if( csFunc == "self.true_dodge" )					lpOperand->bValueType = TYPE_SELF_TRUE_DODGE;
					else if( csFunc == "self.true_r_air" )					lpOperand->bValueType = TYPE_SELF_TRUE_AIR_RESIST;
					else if( csFunc == "self.true_r_fire" )					lpOperand->bValueType = TYPE_SELF_TRUE_FIRE_RESIST;
					else if( csFunc == "self.true_r_water" )				lpOperand->bValueType = TYPE_SELF_TRUE_WATER_RESIST;
					else if( csFunc == "self.true_r_earth" )				lpOperand->bValueType = TYPE_SELF_TRUE_EARTH_RESIST;
					else if( csFunc == "self.true_r_dark" )					lpOperand->bValueType = TYPE_SELF_TRUE_DARK_RESIST;
					else if( csFunc == "self.true_r_light" )				lpOperand->bValueType = TYPE_SELF_TRUE_LIGHT_RESIST;
					else if( csFunc == "self.true_air" )					lpOperand->bValueType = TYPE_SELF_TRUE_AIR_POWER;
					else if( csFunc == "self.true_fire" )					lpOperand->bValueType = TYPE_SELF_TRUE_FIRE_POWER;
					else if( csFunc == "self.true_water" )					lpOperand->bValueType = TYPE_SELF_TRUE_WATER_POWER;
					else if( csFunc == "self.true_earth" )					lpOperand->bValueType = TYPE_SELF_TRUE_EARTH_POWER;
					else if( csFunc == "self.true_dark" )					lpOperand->bValueType = TYPE_SELF_TRUE_DARK_POWER;
					else if( csFunc == "self.true_light" )					lpOperand->bValueType = TYPE_SELF_TRUE_LIGHT_POWER;
					else if( csFunc == "target.true_int" )					lpOperand->bValueType = TYPE_TARGET_TRUE_INT;
					else if( csFunc == "target.true_str" )					lpOperand->bValueType = TYPE_TARGET_TRUE_STR;
					else if( csFunc == "target.true_end" )					lpOperand->bValueType = TYPE_TARGET_TRUE_END;
					else if( csFunc == "target.true_wis" )					lpOperand->bValueType = TYPE_TARGET_TRUE_WIS;
					else if( csFunc == "target.true_agi" )					lpOperand->bValueType = TYPE_TARGET_TRUE_AGI;
					else if( csFunc == "target.true_attack" )				lpOperand->bValueType = TYPE_TARGET_TRUE_ATTACK;
					else if( csFunc == "target.true_dodge" )				lpOperand->bValueType = TYPE_TARGET_TRUE_DODGE;
					else if( csFunc == "target.true_r_air" )				lpOperand->bValueType = TYPE_TARGET_TRUE_AIR_RESIST;
					else if( csFunc == "target.true_r_fire" )				lpOperand->bValueType = TYPE_TARGET_TRUE_FIRE_RESIST;
					else if( csFunc == "target.true_r_water" )				lpOperand->bValueType = TYPE_TARGET_TRUE_WATER_RESIST;
					else if( csFunc == "target.true_r_earth" )				lpOperand->bValueType = TYPE_TARGET_TRUE_EARTH_RESIST;
					else if( csFunc == "target.true_r_dark" )				lpOperand->bValueType = TYPE_TARGET_TRUE_DARK_RESIST;
					else if( csFunc == "target.true_r_light" )				lpOperand->bValueType = TYPE_TARGET_TRUE_LIGHT_RESIST;
					else if( csFunc == "target.true_air" )					lpOperand->bValueType = TYPE_TARGET_TRUE_AIR_POWER;
					else if( csFunc == "target.true_fire" )					lpOperand->bValueType = TYPE_TARGET_TRUE_FIRE_POWER;
					else if( csFunc == "target.true_water" )				lpOperand->bValueType = TYPE_TARGET_TRUE_WATER_POWER;
					else if( csFunc == "target.true_earth" )				lpOperand->bValueType = TYPE_TARGET_TRUE_EARTH_POWER;
					else if( csFunc == "target.true_dark" )					lpOperand->bValueType = TYPE_TARGET_TRUE_DARK_POWER;
					else if( csFunc == "target.true_light" )				lpOperand->bValueType = TYPE_TARGET_TRUE_LIGHT_POWER;
					else if( csFunc == "target.ac" )						lpOperand->bValueType = TYPE_TARGET_AC;
					else if( csFunc == "self.ac" )							lpOperand->bValueType = TYPE_SELF_AC;
					else if( csFunc == "target.type" )						lpOperand->bValueType = TYPE_TARGET_TYPE;
					else if( csFunc == "target.xp" )						lpOperand->bValueType = TYPE_TARGET_XP;
					else if( csFunc == "self.type" )						lpOperand->bValueType = TYPE_SELF_TYPE;
					else if( csFunc == "self.xp" )							lpOperand->bValueType = TYPE_SELF_XP;
					else if( csFunc == "self.gold" )						lpOperand->bValueType = TYPE_SELF_GOLD;
					else if( csFunc == "target.gold" )						lpOperand->bValueType = TYPE_TARGET_GOLD;
					else if( csFunc == "range" )							lpOperand->bValueType = TYPE_RANGE;                          
					else if( csFunc == "infinite" )							lpOperand->bValueType = TYPE_INFINITE;     
					else _keepChecking = true;

					if (!_keepChecking) {
					}else if ( csFunc == "target.karma" )					lpOperand->bValueType = TYPE_TARGET_KARMA;
					else if ( csFunc == "self.karma" )						lpOperand->bValueType = TYPE_SELF_KARMA;
					else if ( csFunc == "currenttime" )						lpOperand->bValueType = TYPE_TIMESTAMP;
					else if ( csFunc == "target.range" )					lpOperand->bValueType = TYPE_RANGE;
					else if ( csFunc == "global.hour" )						lpOperand->bValueType = TYPE_GLOBAL_HOUR;
					else if ( csFunc == "global.minute" )					lpOperand->bValueType = TYPE_GLOBAL_MINUTE;
					else if ( csFunc == "global.second" )					lpOperand->bValueType = TYPE_GLOBAL_SECOND;
					else if( csFunc == "self.weight" )						lpOperand->bValueType = TYPE_SELF_WEIGHT;
					else if( csFunc == "self.maxweight" )					lpOperand->bValueType = TYPE_SELF_MAXWEIGHT;
					else if( csFunc == "target.weight" )					lpOperand->bValueType = TYPE_TARGET_WEIGHT;
					else if( csFunc == "target.maxweight" )					lpOperand->bValueType = TYPE_TARGET_MAXWEIGHT;
					else if( csFunc == "isblocking" )						lpOperand->bValueType = TYPE_ISBLOCKING;
					else if( csFunc == "self.posx" )						lpOperand->bValueType = TYPE_SELF_POSX;
					else if( csFunc == "self.posy" )						lpOperand->bValueType = TYPE_SELF_POSY;
					else if( csFunc == "self.posw" )						lpOperand->bValueType = TYPE_SELF_POSW;
					else if( csFunc == "target.posx" )						lpOperand->bValueType = TYPE_TARGET_POSX;
					else if( csFunc == "target.posy" )						lpOperand->bValueType = TYPE_TARGET_POSY;
					else if( csFunc == "target.posw" )						lpOperand->bValueType = TYPE_TARGET_POSW;
					else if( csFunc == "self.gender" )						lpOperand->bValueType = TYPE_SELF_GENDER;
					else if( csFunc == "target.gender" )					lpOperand->bValueType = TYPE_TARGET_GENDER;
					else if( csFunc == "self.true_maxhp" )					lpOperand->bValueType = TYPE_SELF_TRUE_MAXHP;
					else if( csFunc == "target.true_maxhp" )				lpOperand->bValueType = TYPE_TARGET_TRUE_MAXHP;
					else if( csFunc == "self.appearance" )					lpOperand->bValueType = TYPE_SELF_APPEARANCE;
					else if( csFunc == "target.appearance" )				lpOperand->bValueType = TYPE_TARGET_APPEARANCE;
					else if( csFunc == "self.true_maxmana" )				lpOperand->bValueType = TYPE_SELF_TRUE_MAXMANA;
					else if( csFunc == "target.true_maxmana" )				lpOperand->bValueType = TYPE_TARGET_TRUE_MAXMANA;
					else if( csFunc == "self.luck" )						lpOperand->bValueType = TYPE_SELF_LUCK;
					else if( csFunc == "target.luck" )						lpOperand->bValueType = TYPE_TARGET_LUCK;
					else if( csFunc == "self.true_luck" )					lpOperand->bValueType = TYPE_SELF_TRUE_LUCK;
					else if( csFunc == "target.true_luck" )					lpOperand->bValueType = TYPE_TARGET_TRUE_LUCK;
         			else if ( csFunc == "target.crime" )                    lpOperand->bValueType = TYPE_TARGET_CRIME;
 					else if ( csFunc == "self.crime" )		                lpOperand->bValueType = TYPE_SELF_CRIME;
         			else if ( csFunc == "target.honor" )	                lpOperand->bValueType = TYPE_TARGET_HONOR;
					else if ( csFunc == "self.honor" )	                    lpOperand->bValueType = TYPE_SELF_HONOR;
					else if( csFunc == "self.id" )							lpOperand->bValueType = TYPE_SELF_ID;
					else if( csFunc == "target.id" )						lpOperand->bValueType = TYPE_TARGET_ID;
					else if( csFunc == "target.staticid" )					lpOperand->bValueType = TYPE_TARGET_STATICID;
					else if ( csFunc == "target.crime" )					lpOperand->bValueType = TYPE_TARGET_CRIME;
					else if ( csFunc == "self.crime" )						lpOperand->bValueType = TYPE_SELF_CRIME;
					else if ( csFunc == "target.honor" )					lpOperand->bValueType = TYPE_TARGET_HONOR;
					else if ( csFunc == "self.honor" )						lpOperand->bValueType = TYPE_SELF_HONOR;
					else if( csFunc == "self.statpoints" )					lpOperand->bValueType = TYPE_SELF_STATPOINTS;
					else if( csFunc == "target.statpoints" )				lpOperand->bValueType = TYPE_TARGET_STATPOINTS;
					else if( csFunc == "self.skillpoints" )					lpOperand->bValueType = TYPE_SELF_SKILLPOINTS;
					else if( csFunc == "target.skillpoints" )				lpOperand->bValueType = TYPE_TARGET_SKILLPOINTS;
					else if( csFunc == "self.true_ac" )						lpOperand->bValueType = TYPE_SELF_TRUE_AC;
					else if( csFunc == "target.true_ac" )					lpOperand->bValueType = TYPE_TARGET_TRUE_AC;
					/******************************************************************************/
					// Functions
               ...
               else if( csFunc == "self.viewfriendly" )
               {
                  lpOperand->bFunction = FUNC_SELF_VIEW_FRIENDLY;
                  lpOperand->lpLeftOperand = new OPERAND;                
                  tlOperands.AddToTail( lpOperand->lpLeftOperand );                
                  ...
                  if( !ParseFormula( lpszFormula + i, lpOperand->lpLeftOperand ) )
                  {
                     _LOG_DEBUG
                        LOG_DEBUG_LVL2,
                        "Formula error: invalid monsterid parameter."
                        LOG_
                        boError = TRUE;
                  }
                  /******************************************************************************/
               }
               else if( csFunc == "target.viewfriendly" )
               {
                  lpOperand->bFunction = FUNC_TARGET_VIEW_FRIENDLY;
                  lpOperand->lpLeftOperand = new OPERAND;                
                  tlOperands.AddToTail( lpOperand->lpLeftOperand );                
                  ...
                  /******************************************************************************/
					...
					// IF blocks have 2 more parameters
						...
						if( nFirstParamEnd != -1 && nSecondParamStart != -1 )
						{
							...
							//TRACE( "\r\n-IF( %s ", lpszFirstParam ); //NMNMNM ben trop de log pour rien...
