...
Add GetInViewObjectsUnits() for .TakeALL by NightMare (27/06/2009)
/******************************************************************************/
...
typedef struct _sWorldObjectSpec
{
   Unit *pUnit;
   DWORD dwDropTime;
   DWORD dwLockID;
...
/******************************************************************************/
class __declspec(dllexport) WorldMap : public CLock
/******************************************************************************/
{
public:
	WorldMap();
	...
	void CreateMap(unsigned short x, unsigned short y, unsigned short world, unsigned char *buffy, unsigned char *buffyOri/*, TemplateList <MonsterEncounter> *werw*/
		);
	
	// Grabs (removes and returns) a unit from the world
	//Unit *grab_unit(unsigned short xo, unsigned short yo);	
	// Adds an object to the world
	void deposit_unit(WorldPos where, Unit *obj, DWORD dwLinkID = 0);
   ...
   void valid_link_expire();
   bool valid_link_id( WorldPos where, DWORD ID ,DWORD dwGetUserID);	
   ...
   DWORD GetInViewObjectsUnits(WorldPos wlWhere, BYTE bRange,DWORD *pdwID, WorldPos *pWL);	
   DWORD GetZoneObjectsUnits(int iX1,int iY1,int iX2,int iY2,int iMaxElement,DWORD *pdwID, WorldPos *pWL);
   void  RemoveZoneObjectsUnits(int iX1,int iY1,int iX2,int iY2);
   ...
   BYTE QueryAreaTypeMap( WorldPos wlPos );
...
private:	
	...
	void SetBlocking(WorldPos where, unsigned char which);	
   ...
   unsigned char *map_buffyOri;

   std::map<__int64,sWorldObjectSpec*> m_WorldObjectMap;
