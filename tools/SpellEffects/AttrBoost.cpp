...
/******************************************************************************/
REGISTER_SPELL_EFFECT( ATTRBOOST, AttrBoost::NewFunc, ATTR_BOOST_EFFECT, __noop );
...
/******************************************************************************/
{
    ...
    switch( wParamID )
	{
		...
		case BoostedStat:
			...
			if( SAME( "strength" ) || SAME( "str" ) )				wBoostedStat = STAT_STR;
			else if( SAME( "agility" ) || SAME( "agi" ) )			wBoostedStat = STAT_AGI;
			else if( SAME( "endurance" ) || SAME( "end" ) )         wBoostedStat = STAT_END;
			else if( SAME( "luck" ) || SAME( "lck" ) )	            wBoostedStat = STAT_LCK;
			else if( SAME( "wisdom" ) || SAME( "wis" ) )            wBoostedStat = STAT_WIS;
			else if( SAME( "intelligence" ) || SAME( "int" ) )      wBoostedStat = STAT_INT;
			else if( SAME( "attack skill" ) || SAME( "attack" ) )   wBoostedStat = STAT_ATTACK;
			else if( SAME( "dodge skill" ) || SAME( "dodge" ) )     wBoostedStat = STAT_DODGE;
			else if( SAME( "damage" ) || SAME( "dam" ) )            wBoostedStat = STAT_DAMAGE;
			else if( SAME( "radiance" ) )				            wBoostedStat = STAT_RADIANCE;
			else if( SAME( "mana" ) )						        wBoostedStat = BOOST_MANA;
			else if( SAME( "air resist" ) || SAME( "r_air" ) )	    wBoostedStat = STAT_AIR_RESIST;
			else if( SAME( "water resist" ) || SAME( "r_water" ) )	wBoostedStat = STAT_WATER_RESIST;        
			else if( SAME( "fire resist" ) || SAME( "r_fire" ) )    wBoostedStat = STAT_FIRE_RESIST;
			else if( SAME( "earth resist" ) || SAME( "r_earth" ) )  wBoostedStat = STAT_EARTH_RESIST;
			else if( SAME( "air power" ) || SAME( "air" ) )         wBoostedStat = STAT_AIR_POWER;
			else if( SAME( "water power" ) || SAME( "water" ) )	    wBoostedStat = STAT_WATER_POWER;
			else if( SAME( "fire power" ) || SAME( "fire" ) )       wBoostedStat = STAT_FIRE_POWER;
			else if( SAME( "earth power" ) || SAME( "earth" ) )     wBoostedStat = STAT_EARTH_POWER;
			else if( SAME( "light resist" ) || SAME( "r_light" ) )  wBoostedStat = STAT_LIGHT_RESIST;
			else if( SAME( "dark resist" ) || SAME( "r_dark" ) )	wBoostedStat = STAT_DARK_RESIST;
			else if( SAME( "light power" ) || SAME( "light" ) )		wBoostedStat = STAT_LIGHT_POWER;
			else if( SAME( "dark power" ) || SAME( "dark" ) )       wBoostedStat = STAT_DARK_POWER;
			else if( SAME( "ac" ) )									wBoostedStat = STAT_AC;
			else if( SAME( "max hp" ) )					            wBoostedStat = STAT_MAX_HP;
			else if( SAME( "max mana" ) )				            wBoostedStat = STAT_MAX_MANA;
...
void AttrBoost::CallEffect(SPELL_EFFECT_PROTOTYPE)
/******************************************************************************/
{
   TRACE("***AttrBoost\n");
      ...
      else
      {
         ...
         if( dwDuration != 0 )
         {
            ...
            if( wBoostedStat == STAT_RADIANCE )
            {
               // Send light update.
               TFCPacket sending;
               sending << (RQ_SIZE)RQ_UnitUpdate; //Light changed
...
void AttrBoost::BoostRemoval( EFFECT_FUNC_PROTOTYPE)
/******************************************************************************/
{
   ...
   if( wMessageID == MSG_OnTimer )
   {
      ...
      if( theBoost != NULL )
      {
         ...
         if( updateStatus )
         {
            ...
            if( self->GetType() == U_PC )
            {
               ...
               TFCPacket sendingS2;
               ch->PacketStatus2(sendingS2);
               ch->SendPlayerMessage(sendingS2);
            }
         }
      }
      ...
      DispellEffectStatus( self, lpAddBoost->dwSpellID ,1);
   ...
   else if( wMessageID == MSG_OnDispell ) //now destroy effect directly here
   {
      ...
      // Remove the effect from the client.
      DispellEffectStatus( self, lpAddBoost->dwSpellID ,2);
   ...
   else if( wMessageID == MSG_OnLoadPlayer )
   {
      ...
      else if( lpDataSave->bBufferSize == sizeof( ADD_BOOSTv1 ) )
      {
         ADD_BOOSTv1 v1;            
...
SpellEffect *AttrBoost::NewFunc(LPSPELL_STRUCT lpSpell) // The spell structure for any spell effect registration.
/******************************************************************************/
{
