...
/******************************************************************************/
{
	...
	InitTalk
		...
		CmdAND("BUY","TICKET")  // BUY A TICKET, BUY TICKET
			""
			IF ((DWORD(CheckNPCFlag(__NPC_NAME_LOTTERY_START)) < CurrentRound) && (CheckNPCFlag(__NPC_NAME_LOTTERY_START) != 0 ))
				Conversation
		...
		CmdAND("LOTTERY","TIME") // WHAT IS THE LOTTERY TIME, TIME OF THE LOTTERY, LOTTERY TIME
			...
			IF ((CheckNPCFlag(__NPC_NAME_LOTTERY_START) != 0) && (CheckNPCFlag(__NPC_NAME_LOTTERY_CHECK) != 16) && (DWORD(CheckNPCFlag(__NPC_NAME_LOTTERY_START)) <= CurrentRound))
		...
		Command2(INTL( 6042, "CLAIM"),INTL( 6043, "VALIDATE"))
			...
			;int nLotteryTime = ((CheckNPCFlag(__NPC_NAME_LOTTERY_START) - CurrentRound) / 1200)
			IF ((DWORD(CheckNPCFlag(__NPC_NAME_LOTTERY_ELAPSE_TIME)) <= CurrentRound) && (CheckNPCFlag(__NPC_NAME_LOTTERY_ELAPSE_TIME) != 0) && (CheckNPCFlag(__NPC_NAME_LOTTERY_CHECK) == 16) )
			...
			// * * * PRAY GOD THAT THE USER WON'T SEE THIS * * * 
				Conversation
					INTL( 6048, "{The} {script} {should} {have} {ended} {previously.} {Please} {report} {this} {bug} {to} {Dialsoft} {immediately.}")
			ENDIF
		...
		ParamCmd(INTL( 5242, "PICK $"))
			...
			ELSE
				...
				if (nFirstNumber == nWhatNumberYouPicked) ++nMatches;
				if (nSecondNumber == nWhatNumberYouPicked) ++nMatches;
				if (nThirdNumber == nWhatNumberYouPicked) ++nMatches;
				if (nFourthNumber == nWhatNumberYouPicked) ++nMatches;
				if (nFifthNumber == nWhatNumberYouPicked) ++nMatches;
				Conversation
