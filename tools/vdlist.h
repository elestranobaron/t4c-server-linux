...
class TemplateList : public CLock{
	...
	public:
      ...
      ObjectType *GetObjectByIdx(int iDX);
...
template <class ObjectType>
ObjectType *TemplateList<ObjectType>::GetObjectByIdx( int iDX ){
   Lock();
   //PushList();
   int iCnt = 0;
   ToHead();
...
void TemplateList<ObjectType>::DeleteAbsolute(void) {
	...
	// At both Head and Tail of the List.
		...
		if (TempObject != NULL)
		{
			if (TempObject->Pointer != NULL)
			{
				delete TempObject->Pointer;
				TempObject->Pointer = NULL;
			}
			delete TempObject;
			TempObject = NULL;
		}
