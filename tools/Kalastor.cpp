...
ENDIF
IF (DWORD(CheckNPCFlag( __NPC_FLAG_KALASTOR_INFECTED)) > CurrentRound)
	...
	ELSE                
		...
		BREAK
	ENDIF
ELSEIF ((DWORD(CheckNPCFlag( __NPC_FLAG_KALASTOR_INFECTED )) < CurrentRound) && (CheckNPCFlag( __NPC_FLAG_MURMUNTAG_INFECTED ) != 0 ))
	Conversation
